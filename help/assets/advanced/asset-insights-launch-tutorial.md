...
>[!CAUTION]
>
>請務必下載最 [新版核心元件](https://github.com/adobe/aem-core-wcm-components) ，以進行實作。

視訊使用的核心元件版本為2.2.2，不再是最新版本；請務必使用最新版本，然後再繼續下一節。

* 下載資 [產分析範例影像內容](./assets/asset-insights-launch-tutorial/aem-assets-insights-sample.zip)
* 下載 [最新的AEM WCM核心元件](https://github.com/adobe/aem-core-wcm-components/releases)

## 第二部分：啟用範例影像元件的資產前瞻分析追蹤 {#sample-image-component-asset-insights}
...
核心影像元件 ***使用影像標籤之父&lt;div*** >中的data-asset-id屬性來啟用／停用此功能。 Proxy元件會以下列變更覆寫核心元件。

* 從image. ***html中&lt;img>元素的父div移除data-asset-id***
* 將 ***data-aem-asset-id*** 直接新增至image.html內的&lt;img>元素
* 將 ***data-trackable=&#39;true&#39;*** value新增至image.html中的&lt;img>元素
* ***data-aem-asset-id*** 和 ***data-trackable=&#39;true&#39;*** are destang at some node level

>[!NOTE]
>
>*data-aem-asset-id=&#39;image.UUID&#39;* 和 *data-trackable=&#39;true&#39;* ，是資產曝光數需要顯示的關鍵屬性。 對於資產點按前瞻分析，除了&lt;img>標籤上出現的上述資料屬性外，父&lt;a>標籤必須有有效的href值。
...
* Analytics公司名稱
* Analytics使用者名稱
* 共用密碼(可從 *Adobe Analytics >管理>公司設定>網站服務取得*)。
...
* **\&lt;code>assetAnalytics.core.assetLoaded\&lt;code>** :呼叫。
* **\&lt;code>assetAnalytics.core.assetClicked\&lt;code>** :呼叫時，只有當asset-DOM-element具有父項錨記且具有有效的外部&#39;href&#39;屬性時，才會對asset-DOM-element傳送&#39;click&#39;事件，而此情況相關

最後，Pagetracker將初始化函式實作。

* **\&lt;code>assetAnalytics.dispatcher.init()\&lt;code>** :呼叫，以初始化Pagetracker元件。 必須在從網頁產生任何資產前瞻分析事件（印象和／或點按）之前呼叫此ID。
* **\&lt;code>assetAnalytics.dispatcher.init()\&lt;code>** :（可選）接受AppMeasurement物件— 如果提供，則不會嘗試建立AppMeasurement物件的新例項。
...
### 規則2:影像追蹤器— Action 2(image-tracker.js) {#rule-image-tracker-action-image-tracker-js}
...
您也可以使用下列Chrome擴充功能，將DTM切換至除錯模式： [啟動和DTM交換機](https://chrome.google.com/webstore/detail/launch-and-dtm-switch/nlgdemkdapolikbjimjajpmonpbpmipk?hl=en)。 這可讓您更輕鬆地查看是否有與DTM部署相關的錯誤。 此外，您還可以透過任何瀏覽器的開發人員工具 *-> JS Console* ，手動將DTM切換為除錯模式，方法是新增下列程式碼片段：

## 第五部分：測試分析追蹤和同步分析資料{#analytics-tracking-asset-insights}
