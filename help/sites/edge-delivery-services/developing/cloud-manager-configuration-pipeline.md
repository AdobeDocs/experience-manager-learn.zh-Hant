---
title: 使用Edge Delivery設定管道部署設定
description: 瞭解如何在Cloud Manager中建立和執行Edge Delivery設定管道。
version: Experience Manager as a Cloud Service
feature: Edge Delivery Services
topic: Development
role: Admin, Developer
level: Beginner
doc-type: Technical Video
duration: 120
last-substantial-update: 2025-12-10T00:00:00Z
jira: KT-18126
source-git-commit: 6acf7f1d5c91b29a64b372b678f04b7cb880039b
workflow-type: tm+mt
source-wordcount: '75'
ht-degree: 1%

---


# Edge Delivery設定管道

探索如何在Adobe [Cloud Manager](https://my.cloudmanager.adobe.com)中建立並執行Edge Delivery設定管道，允許受控部署設定，例如CDN規則、WAF篩選器或記錄轉送。

>[!VIDEO](https://video.tv.adobe.com/v/3458975/?learn=on&enablevpops)

如需設定選項和格式的完整總覽，請參閱[Edge Delivery設定管道檔案](https://experienceleague.adobe.com/zh-hant/docs/experience-manager-cloud-service/content/operations/config-pipeline#yamls-for-eds)。
