...
**使用分類法作為同義字字搜尋。**當使用者搜尋關鍵字時，系統會建立搜尋所有同義字的第二個搜尋。
此外，系統可以提供關鍵字清單來改善一致性，而不必手動輸入「car」。

**使用分類法作為字典。**&#x200B;您可以展開單一標籤並使用標籤的所有同義字，而不只是列印「car」。

**多個類別。**&#x200B;與資料夾階層相反，標籤可用於同時表示多個分類。 資產標籤有：
...
**標準化標籤。**&#x200B;建立提供權威辭彙的字彙表。 若不建立標準，重複就會造成問題。 此外，建議不僅稽核分類，也稽核標籤的使用情況。

**請勿過度標籤。如果過於頻繁地分佈，**&#x200B;標籤可能會失去其重要性。剪除無關的標籤以獲得最佳效率。

**在一段時間內重新評估標籤。**&#x200B;請記住，商務術語和業務內容很少保持靜態。 您可能會發現有必要重新標準化及重新套用標籤。
...
**利用中繼資料**&#x200B;加強協同合作：利用AdobeAsset Link、Adobe Bridge和AEM Desktop將創意流程繫結在一起，並利用中繼資料簡化創意工作流程。 使用這些工具將豐富您整個創意流程的中繼資料和使用者體驗。
...
**描述** — 如果資料描述資產或內容片段，則資料應是附加中繼資料的一部分。

**搜尋** — 如果要在搜尋中使用中繼資料，則必須附加該中繼資料。

**曝光** — 如果您要向協力廠商公開發佈平台上的中繼資料，請小心不要同時公開「內部」中繼資料。

**持續時間** — 中繼資料的存留時間越長，就越可能是附加中繼資料的良好候選者。

**相關的業務流程** — 在中繼資料中擁有永久產品ID肯定有幫助。 但與產品目錄相關的專案類別，是資產的可疑中繼資料。

**組織與處理** — 如果中繼資料的性質是組織性質，例如核准工作流程中的狀態或特定部門的所有權，則應考慮外部中繼資料，而非將中繼資料附加至資產。
