---
title: 使用Platform Web SDK整合Adobe Analytics
description: 瞭解如何使用Platform Web SDK整合Adobe Experience Manager (AEM)和Adobe Analytics的現代方法。 本教學課程會引導您完成收集頁面檢視和CTA點按資料，以在Adobe Analytics Workspace中取得資料分析。
...
# 使用Platform Web SDK整合Adobe Analytics

瞭解 **現代方法** 有關如何使用Platform Web SDK整合Adobe Experience Manager (AEM)和Adobe Analytics。 此全方位的教學課程會引導您完成順暢收集資料的程式 [WKND](https://github.com/adobe/aem-guides-wknd#aem-wknd-sites-project) pageview和CTA點按資料。 在AdobeAnalysis Workspace中將收集的資料視覺化，以便探索各種量度和維度，進而獲得有價值的深入分析。 此外，請探索Platform資料集以驗證和分析資料。 加入我們的歷程，利用AEM和Adobe Analytics的強大功能，進行資料導向式決策。

## 概觀

深入瞭解使用者行為是每個行銷團隊的重要目標。 透過瞭解使用者如何與其內容互動，團隊可以做出明智的決策、最佳化策略並取得更好的結果。 WKND行銷團隊是虛構的實體，已瞄準在網站上實作Adobe Analytics來達成此目標。 主要目標是收集兩個關鍵量度的資料：頁面檢視和首頁行動號召(CTA)點選。

透過追蹤頁面檢視，團隊能夠分析哪些頁面最受使用者注意。 此外，追蹤首頁CTA點按可針對團隊的召喚行動元素的有效性提供有價值的深入分析。 此資料可能會揭示哪些CTA正在與使用者產生共鳴、哪些需要調整，並可能發掘提升使用者參與度和促進轉換的新機會。
...
在深入探討AEM與Analytics使用Platform Web SDK的整合程式之前，讓我們先瞭解 _重述重要元件和關鍵要素_ 建立於 [整合Experience PlatformWeb SDK](./web-sdk.md) 教學課程。 為整合提供堅實的基礎。

>[!VIDEO](https://video.tv.adobe.com/v/3419873?quality=12&learn=on)

回顧XDM結構描述、資料流、資料集、標籤屬性以及AEM和標籤屬性連線後，讓我們開始整合歷程。

## 定義Analytics解決方案設計參考(SDR)檔案

在實作過程中，建議建立解決方案設計參考(SDR)檔案。 此檔案是定義業務需求和設計有效資料收集策略的藍圖，扮演著關鍵角色。
...
第一步是設定Adobe Analytics，特別是含有轉換變數(或eVar)和成功事件的報表套裝。 轉換變數可用來測量因果關係。 成功事件可用來追蹤動作。

在本教學課程中，  `eVar5, eVar6, and eVar7` track  _WKND頁面名稱、WKND CTA ID和WKND CTA名稱_ 分別，和 `event7` 用於追蹤  _WKND CTA點選事件_.
...
資料串流會指示Platform Edge Network將收集到的資料傳送至何處。 在 [先前的教學課程](./web-sdk.md)，資料串流會設定為傳送資料給Experience Platform。 此資料流已更新，可將資料傳送至中設定的Analytics報表套裝 [以上](#setup-analytics---report-suite-analysis-workspace) 步驟。

>[!VIDEO](https://video.tv.adobe.com/v/3419876?quality=12&learn=on)

## 建立XDM結構描述

Experience Data Model (XDM)結構描述可幫助您將收集的資料標準化。 在 [先前的教學課程](./web-sdk.md)，此XDM結構描述具有 `AEP Web SDK ExperienceEvent` 欄位群組已建立。 此外，使用此XDM結構描述會建立資料集，以將收集的資料儲存在Experience Platform中。

不過，此XDM結構描述沒有Adobe Analytics特定的欄位群組來傳送eVar事件資料。 會建立新的XDM結構描述，而非更新現有結構描述以避免將eVar事件資料儲存在平台中。
...
在 [先前的教學課程](./web-sdk.md)，即會建立標籤屬性，其中包含資料元素和規則，用以收集、對應和傳送pageview資料。 必須為以下專案增強此功能：
...
+ 若要驗證PageView和HomePage CTA的XDM事件資料，請按一下，使用擴充功能中的Experience PlatformWeb SDK功能表選項。
...
資料集是資料集合的儲存和管理結構，例如遵循結構描述的資料庫表格。 在中建立的資料集 [先前的教學課程](./web-sdk.md) 用於驗證pageview和CTA點選資料是否已擷取到Experience Platform資料集中。 在資料集UI中，各種詳細資訊（例如記錄總數、大小和擷取的批次）會連同視覺上吸引人的長條圖一起顯示。
...
實施Adobe Analytics對於行銷團隊深入瞭解使用者行為、做出明智決策、讓行銷團隊能夠最佳化其內容並做出資料導向式決策至關重要。
...
+ [搭配核心元件使用Adobe Client Data Layer](https://experienceleague.adobe.com/docs/experience-manager-learn/sites/integrations/adobe-client-data-layer/data-layer-overview.html)
