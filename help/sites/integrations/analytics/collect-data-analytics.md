...
description: 使用事件導向的Adobe用戶端資料層，收集使用Adobe Experience Manager建置之網站上使用者活動的相關資料。 了解如何使用標籤規則來監聽這些事件，並將資料傳送至Adobe Analytics報表套裝。
...
了解如何使用 [Adobe用戶端資料層與AEM核心元件](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/developing/data-layer/overview.html) 收集Adobe Experience Manager Sites中某頁面的相關資料。 [Experience Platform](https://experienceleague.adobe.com/docs/experience-platform/tags/home.html) 和 [Adobe Analytics擴充功能](https://experienceleague.adobe.com/docs/experience-platform/tags/extensions/client/analytics/overview.html) 可用來建立規則，以將頁面資料傳送至Adobe Analytics。

## 您要建置的 {#what-build}

![頁面資料追蹤](assets/collect-data-analytics/analytics-page-data-tracking.png)

在本教學課程中，您將根據Adobe用戶端資料層的事件觸發標籤規則。 同時，新增規則觸發時間的條件，然後傳送 **頁面名稱** 和 **頁面範本** 值(AEM頁面至Adobe Analytics)。

### 目標 {#objective}

1. 在標籤屬性中建立從資料層擷取變更的事件導向規則
...
* [Experience Platform偵錯器](https://experienceleague.adobe.com/docs/platform-learn/data-collection/debugger/overview.html) 瀏覽器擴充功能。 本教學課程中的螢幕擷取畫面是從Chrome瀏覽器擷取。
* （選用）AEM Site與 [Adobe用戶端資料層已啟用](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/developing/data-layer/overview.html#installation-activation). 本教學課程使用公開對應 [WKND](https://wknd.site/us/en.html) 網站，但歡迎您使用自己的網站。

>[!NOTE]
>
> 整合標籤屬性和AEM網站需要協助嗎？ [請觀看此影片系列](../experience-platform/data-collection/tags/overview.md).

## 切換WKND站點的標籤環境

此 [WKND](http://wknd.site/us/en.html) 是以 [開放原始碼專案](https://github.com/adobe/aem-guides-wknd) 設計為參考， [教學課程](https://experienceleague.adobe.com/docs/experience-manager-learn/getting-started-wknd-tutorial-develop/overview.html?lang=zh-Hant) (AEM實作)。

您不必設定AEM環境並安裝WKND程式碼基底，而是可以使用Experience Platform偵錯器，以 **開關** 現場 [WKND站點](http://wknd.site/us/en.html) to *您的* 標籤屬性。 不過，如果您的AEM網站已具備 [Adobe用戶端資料層已啟用](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/developing/data-layer/overview.html#installation-activation).

1. 登入Experience Platform和 [建立標籤屬性](https://experienceleague.adobe.com/docs/platform-learn/implement-in-websites/configure-tags/create-a-property.html) （如果尚未）。
1. 確認初始標籤JavaScript [程式庫已建立](https://experienceleague.adobe.com/docs/experience-platform/tags/publish/libraries.html#create-a-library) 並升級為標籤 [環境](https://experienceleague.adobe.com/docs/experience-platform/tags/publish/environments/environments.html).
1. 從程式庫發佈到的標籤環境中複製JavaScript內嵌程式碼。

   ![複製標籤屬性內嵌程式碼](assets/collect-data-analytics/launch-environment-copy.png)

1. 在您的瀏覽器中，開啟新標籤並導覽至 [WKND站點](http://wknd.site/us/en.html)
...
1. 導覽至 **Experience Platform標籤** > **設定** 和 **插入的內嵌程式碼** 將現有的內嵌程式碼取代為 *您的* 從步驟3複製的內嵌程式碼。
...
1. 導覽至 [WKND站點](http://wknd.site/us/en.html).
...
1. 展開回應並檢查 `page` 的下界。 您應會看到下列資料結構：
   ...
   若要將頁面資料傳送至Adobe Analytics，請使用標準屬性，如 `dc:title`, `xdm:language`，和 `xdm:template` 資料層。

   如需詳細資訊，請檢閱 [頁面結構](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/developing/data-layer/overview.html#page) 從核心元件資料結構。

   >[!NOTE]
   >
   > 如果您沒有看到 `adobeDataLayer` JavaScript物件？ 確保 [Adobe客戶端資料層已啟用](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/developing/data-layer/overview.html#installation-activation) 在您的網站上。

## 建立頁面載入規則

Adobe用戶端資料層是 **事件** 驅動資料層。 載入AEM頁面資料層時，會觸發 `cmp:show` 事件。 建立規則，此規則會在 `cmp:show` 事件是從頁面資料層引發。
...
1. 按一下 **開啟編輯器** 在主面板中，輸入下列程式碼片段：
   ...
   上述程式碼片段透過 [推送函式](https://github.com/adobe/adobe-client-data-layer/wiki#pushing-a-function) 進入資料層。 當 `cmp:show` 事件已觸發 `pageShownEventHandler` 函式時，才會呼叫。 在此函式中，會新增一些健全性檢查，並新增 `event` 是使用最新 [資料層的狀態](https://github.com/adobe/adobe-client-data-layer/wiki#getstate) 用於觸發事件的元件。
...
1. 按一下 **開啟編輯器** 在主面板中，輸入下列程式碼片段：
   ...
   此 `event` 物件是從 `trigger()` 方法在自訂事件中呼叫。 此處 `component` 是從資料層衍生的目前頁面 `getState` 在自訂事件中。
...
接著，建立數個資料元素，以從Adobe用戶端資料層擷取不同值。 如先前練習所示，您可以直接透過自訂程式碼存取資料層的屬性。 使用資料元素的優點，是可在各標籤規則間重複使用。
...
1. 在 **程式庫管理** > **報表套裝**，輸入您要用於每個標籤環境的報表套裝id。
   ...
   >[!TIP]
   >
   >建議您使用 *為我管理程式庫選項* 作為「程式庫管理」設定，因為它可讓 `AppMeasurement.js` 程式庫為最新。

1. 核取方塊以啟用 **使用Activity Map**.

   ![啟用使用Activity Map](assets/track-clicked-component/analytic-track-click.png)

1. 在 **一般** > **追蹤伺服器**，請輸入追蹤伺服器，例如 `tmd.sc.omtrdc.net`. 如果您的網站支援，請輸入SSL追蹤伺服器 `https://`
...
接下來，更新 **已載入頁面** 規則來使用 **元件資源類型** 資料元素，以確保規則只在 `cmp:show` 事件適用於 **頁面**. 其他元件可以觸發 `cmp:show` 事件，例如，當投影片變更時，輪播元件就會觸發它。 因此，請務必為此規則新增條件。

1. 在標籤屬性UI中，導覽至 **已載入頁面** 規則。
1. 在 **條件** 按一下 **新增** 開啟 **條件設定** 嚮導。
1. 針對 **條件類型** 欄位，選擇 **值比較** 選項。
...
目前 **已載入頁面** 規則只會輸出console陳述式。 接下來，使用資料元素和Analytics擴充功能，將Analytics變數設為 **動作** 在 **已載入頁面** 規則。 我們也會設定額外的動作來觸發 **頁面檢視信標** 並將收集的資料傳送至Adobe Analytics。

1. 在頁面載入規則中， **移除** the **核心 — 自訂程式碼** 動作（控制台陳述式）:
...
1. 在主面板中，選取可用的 **eVar** 並設為資料元素的值 **頁面範本**. 使用資料元素圖示 ![資料元素圖示](assets/collect-data-analytics/cylinder-icon.png) ，選擇 **頁面範本** 元素。
...
1. 設定 **擴充功能** 類型 **Adobe Analytics** 並設定 **動作類型** to  **傳送信標**. 由於此動作會被視為頁面檢視，請將預設追蹤設為 **`s.t()`**.
...
1. 確認Debugger將標籤屬性對應至 *您的* 開發環境，如先前和 **主控台記錄** 已勾選。
...
您只是在Experience Platform中使用事件導向的Adobe用戶端資料層和標籤，從AEM網站收集資料頁面資料，並將其傳送至Adobe Analytics。
