...
對於偏好CSS的使用者，此程式碼片段下方是此[!DNL LESS]編譯為的CSS。
...
以上[!DNL LESS]是以Experience Manager為下列CSS的方式原生編譯。
...
* 公開更多資料並隱藏它比公開太少的資料要好，這些資料需要未來的後端開發才能公開它。
   ...
   * 實作可供製作的內容切換有助於保持此HTML的精細度，讓作者能夠選取要將哪些內容元素寫入HTML。 將影像寫入HTML時，可能不會用於所有樣式時，尤其重要。
   * 此規則的例外情況是，昂貴的資源（例如影像）預設會公開，因為CSS隱藏的事件影像在此情況下將會不必要地擷取。
...
>[!NOTE]
>
>樣式系統使[BEM](https://en.bem.info/)出現小的技術差異，因為`BLOCK`和`BLOCK--MODIFIER`未應用到由[BEM](https://en.bem.info/)指定的相同元素。
>
>相反地，由於產品限制，`BLOCK--MODIFIER`會套用至`BLOCK`元素的父項。
>
>[BEM](https://en.bem.info/)的所有其他租戶都應與對齊。

* 使用前置處理器，如[LESS](https://lesscss.org/)(AEM原生支援)或[SCSS](https://sass-lang.com/)（需要自訂建置系統），以允許清除CSS定義和重新可用。

* 保持選擇器重量/特異性一致；這有助於避免和解決難以識別的CSS階層衝突。
* 將每個樣式組織為離散檔案。
   * 這些檔案可以使用LESS/SCSS `@imports`組合，或者如果需要原始CSS，則可以通過HTML客戶端庫檔案包含或自定義前端資產構建系統組合。
* 避免混合許多複雜的樣式。
...
* 當`BLOCK--MODIFIER`附加至回應式格線時，請避免直接使用樣式。 更改此元素的顯示可能會影響響應網格的呈現和功能，因此，當更改響應網格的行為時，只有在此級別上的樣式。
* 使用`BLOCK--MODIFIER`應用樣式範圍。 `BLOCK__ELEMENT--MODIFIERS`可在元件中使用，但由於`BLOCK`代表元件，而元件是已設定樣式的元件，因此樣式是「已定義」，並通過`BLOCK--MODIFIER`限定範圍。
