...
將使用與這些值匹配的第一個策略。 如果找不到，則 [!DNL CORS] 請求將被拒絕。

如果根本未配置策略， [!DNL CORS] 由於處理程式將被禁用，因此會被有效拒絕，因此也不會回應請求，只要伺服器的其他模組沒有響應 [!DNL CORS]。
...
可能需要完全清除快取，以確保在下次請求後，頭被適當快取 `/cache/headers` 配置更新。
