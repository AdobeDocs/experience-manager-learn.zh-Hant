...
在本章中，讓我們探討基本頁元件與可編輯模板之間的關係。 學習根據某些模型構建無風格文章模板 [Adobe XD](https://helpx.adobe.com/support/xd.html)。 在構建模板的過程中，將介紹「可編輯模板」的核心元件和高級策略配置。
...
在本教程的這一部分中，您將構建一個新的「文章頁面模板」，該模板可用於建立文章頁面並與公共結構對齊。 文章頁面模板基於設計和Adobe XD生產的UI套件。 本章僅側重於構建模板的結構或骨架。 未實現樣式，但模板和頁面功能正常。

![文章頁面設計和未設樣式的版本](assets/pages-templates/what-you-will-build.png)

## UI規劃與Adobe XD {#adobexd}

通常，新網站的規劃首先是模型和靜態設計。 [Adobe XD](https://helpx.adobe.com/support/xd.html) 是構建用戶體驗的設計工具。 接下來，讓我們檢查一個UI工具包和模型，以幫助規劃文章頁面模板的結構。
...
建立頁面時，必須選擇模板，該模板將用作建立頁面的基礎。 模板定義結果頁面的結構、初始內容和允許的元件。

有三個主要領域 [可編輯模板](https://experienceleague.adobe.com/docs/experience-manager-65/developing/platform/templates/page-templates-editable.html):

1. **結構**  — 定義作為模板一部分的元件。 這些內容作者不可編輯。
1. **初始內容**  — 定義模板開始的元件，內容作者可以編輯和/或刪除這些元件
1. **策略**  — 定義元件行為方式和作者擁有的選項的配置。

接下來，在中創AEM建與模型結構匹配的模板。 這在的本地實例中發AEM生。 按照以下視頻中的步驟操作：
...
1. 添加 **體驗片段** 要用作的元件 **標題** 的下界。
   ...
   * 將策略設定為 **頁眉** 確保 **預設元素** 設定為 `header`。 的 `header`元素在下一章中與CSS對應。
1. 添加 **體驗片段** 要用作的元件 **頁腳** 的下界。
   * 配置元件以指向 `/content/experience-fragments/wknd/us/en/site/footer/master`。
   * 將策略設定為 **頁腳** 確保 **預設元素** 設定為 `footer`。 的 `footer` 元素在下一章中與CSS對應。
1. 鎖定 **主** 建立模板時包含的容器。
   * 將策略設定為 **頁面首頁** 確保 **預設元素** 設定為 `main`。 的 `main` 元素在下一章中與CSS對應。
1. 添加 **影像** 元件 **主** 容器。
   * 解鎖 **影像** 元件。
1. 添加 **麵包屑** 元件下方 **影像** 元件。
   * 為 **麵包屑** 元件命名 **文章頁 — Breadcrumb**。 設定 **導航起始級別** 至 **4**。
1. 添加 **容器** 元件下方 **麵包屑** 元件和內部 **主** 容器。 這起作用 **內容容器** 的子菜單。
   * 解鎖 **內容** 容器。
   * 將策略設定為 **頁面內容**。
1. 添加其他 **容器** 元件下方 **內容容器**。 這起作用 **側軌** 的下界。
   * 解鎖 **側軌** 容器。
   * 建立名為 **文章頁 — 側欄**。
   * 配置 **允許的元件** 在 **WKND站點項目 — 內容** 要包括： **按鈕**。 **下載**。 **影像**。 **清單**。 **分隔符**。 **社交媒體共用**。 **文本**, **標題**。
1. 更新「頁根」容器的策略。 這是模板上最外層的容器。 將策略設定為 **頁根**。
   * 下 **容器設定**&#x200B;的子菜單。 **佈局** 至 **響應網格**。
1. 為 **內容容器**。 從右到左拖動手柄，將容器縮小為八列寬。
1. 為 **側軌集裝箱**。 從右到左拖動手柄，將容器縮小為四列寬。 然後，將左手柄從左到右拖動一列，使容器3列變寬，並在 **內容容器**。
...
1. 切換到 **初始內容** 的子菜單。
1. 添加 **標題** 元件 **內容容器**。 這是文章標題。 當它為空時，它將自動顯示當前頁面的標題。
1. 添加秒 **標題** 元件。
   * 使用文本配置元件：「作者」。 這是文本佔位符。
   * 將類型設定為 `H4`。
...
1. 更新Web變體模板，該模板用於以下位置的體驗片段 [http://localhost:4502/editor.html/conf/wknd/settings/wcm/templates/xf-web-variation/structure.html](http://localhost:4502/editor.html/conf/wknd/settings/wcm/templates/xf-web-variation/structure.html)
   * 更新策略 **容器** 的下界。
   * 將策略設定為 **XF根**。
   * 在， **允許的元件** 選擇元件組 **WKND站點項目 — 結構** 包括 **語言導航**。 **導航**, **快速搜索** 元件。
...
1. 添加 **WKND Dark徽標** 作為影像 **容器**。 徽標包含在上一步驟中安裝的軟體包中。
   * 修改佈局 **WKND Dark徽標** 要 **二** 欄寬。 從右到左拖動手柄。
   * 配置徽標 **備選文本** &quot;WKND徽標&quot;
   * 將徽標配置為 **連結** 至 `/content/wknd/us/en` 的上界。
1. 配置 **導航** 已放在頁面上的元件。
   * 設定 **排除根級別** 至 **1**。
   * 設定 **導航結構深度** 至 **1**。
   * 修改佈局 **導航** 元件 **八** 欄寬。 從右到左拖動手柄。
1. 刪除 **語言導航** 元件。
1. 修改佈局 **搜索** 元件 **二** 欄寬。 從右到左拖動手柄。 現在，所有元件應在一行上水準對齊。
...
1. 添加 **WKND Light徽標** 作為影像 **容器**。 徽標包含在上一步驟中安裝的軟體包中。
   * 修改佈局 **WKND Light徽標** 要 **二** 欄寬。 從右到左拖動手柄。
   * 配置徽標 **備選文本** &quot;WKND標識燈&quot;
   * 將徽標配置為 **連結** 至 `/content/wknd/us/en` 的上界。
...
接下來，使用「文章頁面」模板建立頁面。 編寫頁面內容以匹配網站模型。 按照以下視頻中的步驟操作：
...
1. 配置 **清單** 元件。
   ...
   * 在， **項設定** 檢查 **連結項** 檢查 **顯示日期**。

## Inspect節點結構 {#node-structure}

此時，文章頁面明顯沒有樣式。 但基本結構已經到位。 接下來，檢查文章頁面的節點結構，以更好地瞭解模板、頁面和元件的作用。
...
1. 按一下 `jcr:content` 節點下 `la-skateparks` 頁並查看屬性：
   ...
   注意 `cq:template`，指向 `/conf/wknd/settings/wcm/templates/article-page`，文章頁面模板。
...
1. 接下來，在 `/apps/wknd/components/page`。 在CRXDE Lite中查看元件屬性：
   ...
   只有兩個HTL指令碼， `customfooterlibs.html` 和 `customheaderlibs.html` 頁面元件下。 *那麼，這個元件如何呈現頁面？*

   的 `sling:resourceSuperType` 屬性點 `core/wcm/components/page/v2/page`。 此屬性允許WKND的頁元件繼承 **全部** 核心元件頁面元件的功能。 這是第一個例子 [代理元件模式](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/developing/guidelines.html#ProxyComponentPattern)。 可以找到更多資訊 [這裡](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/developing/guidelines.html)。

1. InspectWKND元件中的另一個元件， `Breadcrumb` 元件自： `/apps/wknd/components/breadcrumb`。 注意 `sling:resourceSuperType` 可以找到屬性，但這次它指向 `core/wcm/components/breadcrumb/v2/breadcrumb`。 這是使用代理元件模式包括核心元件的另一個示例。 事實上，WKND代碼庫中的所有元件都是核心元件的代AEM理（自定義演示HelloWorld元件除外）。 最佳做法是盡可能多地重複使用核心元件的功能 *先* 編寫自定義代碼。

1. 下一步，檢查「核心元件」頁，位於 `/libs/core/wcm/components/page/v2/page` 使用CRXDE Lite:

   >[!NOTE]
   >
   > 在中，AEM 6.5/6.4核心元件位於 `/apps/core/wcm/components`。 在，AEMas a Cloud Service，核心元件位於 `/libs` 並自動更新。

   ![「核心元件」頁](assets/pages-templates/core-page-component-properties.png)

   請注意，此頁下麵包含許多指令碼檔案。 「核心元件」頁包含許多功能。 此功能被分成多個指令碼，以便更方便維護和可讀性。 通過開啟 `page.html` 尋找 `data-sly-include`:
   ...
   將HTL拆分為多個指令碼的另一個原因是允許代理元件覆蓋單個指令碼以實現自定義業務邏輯。 HTL指令碼 `customfooterlibs.html`, `customheaderlibs.html`，建立的目的是通過實施項目覆蓋。
...
1. Inspect是另一個核心元件，比如 `/libs/core/wcm/components/breadcrumb/v2/breadcrumb`。 查看 `breadcrumb.html` 指令碼，瞭解如何最終生成Breadcrumb元件的標籤。

## 將配置保存到原始碼管理 {#configuration-persistence}

通常，尤其是在項目開始AEM時，將配置（如模板和相關內容策略）保留到原始碼管理是非常有價值的。 這可確保所有開發人員都針對同一組內容和配置工作，並可確保環境之間的更多一致性。 一旦項目達到一定的成熟度，模板管理的做法就可以轉給特定的用戶群。


目前，模板被視為其他代碼，並同步 **文章頁面模板** 作為項目的一部分。
直到現在，代碼從項AEM目推送到的本地實AEM例。 的 **文章頁面模板** 直接在的本地實例上創AEM建，因此需要 **導入** 模AEM板。 的 **ui.content** 模組包含在項AEM目中，用於此特定用途。

在VSCode IDE中，使用 [VSCode同AEM步](https://marketplace.visualstudio.com/items?itemName=yamato-ltd.vscode-aem-sync&amp;ssr=false#overview) 插件。 但他們可能正在使用您配置為 **導入** 或從的本地實例導入內AEM容。

1. 在中，VSCode開啟 `aem-guides-wknd` 項目。
...
1. 重複導入內容的步驟，但選擇 **策略** 資料夾 `/conf/wknd/settings/wcm/policies`。
...
1. Inspect `filter.xml` 檔案 `ui.content/src/main/content/META-INF/vault/filter.xml`。
   ...
   的 `filter.xml` 檔案負責標識隨軟體包一起安裝的節點的路徑。 注意 `mode="merge"` 在指示不要修改的現有內容的每個篩選器上，只添加新內容。 由於內容作者可能正在更新這些路徑，因此代碼部署必須執行 **不** 覆蓋內容。 查看 [FileVault文檔](https://jackrabbit.apache.org/filevault/filter.html) 的子菜單。
   ...
   >[!WARNING]
   >
   > 為確保WKND參考站點的一致部署，已設定項目的某些分支，以便 `ui.content` 覆蓋JCR中的任何更改。 這是按設計進行的，即針對解決方案分支，因為代碼/樣式是針對特定策略編寫的。

## 恭喜！ {#congratulations}

恭喜，您已與Adobe Experience Manager Sites建立了模板和頁面。

### 後續步驟 {#next-steps}

此時，文章頁面明顯沒有樣式。 關注 [客戶端庫和前端工作流](client-side-libraries.md) 教程，瞭解包括CSS和JavaScript在內的最佳實踐，以將全局樣式應用到站點並整合專用的前端構建。

查看完成的代碼 [GitHub](https://github.com/adobe/aem-guides-wknd) 或在Git分支上本地查看和部署代碼 `tutorial/pages-templates-solution`。
