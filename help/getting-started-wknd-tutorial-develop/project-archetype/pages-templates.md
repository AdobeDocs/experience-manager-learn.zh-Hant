...
本章將探討基本頁面元件與可編輯範本之間的關係。 我們會根據 [AdobeXD](https://www.adobe.com/products/xd.html). 在建立範本的過程中，將介紹可編輯模板的核心元件和高級策略配置。
...
您一律可以在上檢視完成的程式碼 [GitHub](https://github.com/adobe/aem-guides-wknd/tree/tutorial/pages-templates-solution) 或切換到分支，本地檢出代碼 `tutorial/pages-templates-solution`.
...
在本教學課程的本節中，您將建立新的「文章頁面範本」，以用來建立新的文章頁面，並與通用結構對齊。 「文章頁面範本」以AdobeXD中產生的設計和UI套件為基礎。 本章僅側重於構建模板的結構或骨架。 未實作樣式，但範本和頁面可運作。

![文章頁面設計與未設定樣式的版本](assets/pages-templates/what-you-will-build.png)

## 使用Adobe XD規劃UI {#adobexd}

在大多數情況下，規劃新網站的開始會是模型和靜態設計。 [Adobe XD](https://www.adobe.com/products/xd.html) 是建立使用者體驗的設計工具。 接下來，我們將檢查UI套件和模型，以協助規劃文章頁面範本的結構。
...
共有3個主要區域 [可編輯的範本](https://experienceleague.adobe.com/docs/experience-manager-65/developing/platform/templates/page-templates-editable.html):
...
1. 新增 **容器** 元件下方 **階層連結** 元件和內部 **main** 容器。 這將作為 **內容容器** ，以取得Advertising Cloud的說明。
   * 解鎖 **內容** 容器。
   * 將策略設定為 **頁面內容**.
1. 添加其他 **容器** 元件下方 **內容容器**. 這將作為 **側邊欄** 容器。
   * 解鎖 **側邊欄** 容器。
   * 建立新策略，命名為 **文章頁面 — 側邊欄**.
...
1. 接合佈局模式 **內容容器**. 從右到左拖動手柄，並將容器縮小為8列寬。
1. 接合佈局模式 **側邊欄容器**. 從右到左拖動手柄，並將容器縮小為4列寬。 然後，從左到右拖動左手柄1列，使容器3列變寬，並在容器3列之間留有1列間隙 **內容容器**.
1. 開啟移動模擬器並切換到移動斷點。 再次接合佈局模式，並建立 **內容容器** 和 **側邊欄容器** 頁面的全寬。 這會在行動斷點中垂直堆疊容器。
1. 更新 **文字** 元件(位於 **內容容器**.
   * 將策略設定為 **內容文字**.
...
1. 更新網頁變異範本，此範本是用於的體驗片段，位於 [http://localhost:4502/editor.html/conf/wknd/settings/wcm/templates/xf-web-variation/structure.html](http://localhost:4502/editor.html/conf/wknd/settings/wcm/templates/xf-web-variation/structure.html)
   * 將策略更新為 **容器** 元件。
   * 將策略設定為 **XF根**.
   * 在 **允許的元件** 選擇元件組 **WKND Sites專案 — 結構** 包含 **語言導覽**, **導覽**，和 **快速搜尋** 元件。
...
1. 新增 **WKND深色徽標** 作為影像 **容器**. 此標誌包含在先前步驟中安裝的套件中。
   * 修改 **WKND深色徽標** 為 **2** 欄寬。 從右到左拖動控點。
   * 使用 **替代文字** 「WKND徽標」。
   * 將標誌配置為 **連結** to `/content/wknd/us/en` 首頁。
1. 設定 **導覽** 已放置在頁面上的元件。
   * 設定 **排除根層級** to **1**.
   * 設定 **導覽結構深度** to **1**.
   * 修改 **導覽** 元件 **8** 欄寬。 從右到左拖動控點。
1. 移除 **語言導覽** 元件。
1. 修改 **搜尋** 元件 **2** 欄寬。 從右到左拖動控點。 現在，所有元件應在單一列上水準對齊。
...
1. 新增 **WKND Light徽標** 作為影像 **容器**. 此標誌包含在先前步驟中安裝的套件中。
   * 修改 **WKND Light徽標** 為 **2** 欄寬。 從右到左拖動控點。
   * 使用 **替代文字** 「WKND標誌燈」。
   * 將標誌配置為 **連結** to `/content/wknd/us/en` 首頁。
1. 新增 **導覽** 元件。 設定 **導覽** 元件：
   * 設定 **排除根層級** to **1**.
...
1. 按一下 `jcr:content` 節點下方 `la-skateparks` 頁面並檢視屬性：
   ...
   請注意 `cq:template`，指向 `/conf/wknd/settings/wcm/templates/article-page`，即我們先前建立的「文章頁面範本」 。
...
1. 下一步，在檢查頁面元件 `/apps/wknd/components/page`. 在CRXDE Lite中查看元件屬性：
   ...
   請注意，只有2個HTL指令碼， `customfooterlibs.html` 和 `customheaderlibs.html` 頁面元件下方。 *那麼，此元件如何呈現頁面？*

   此 `sling:resourceSuperType` 屬性指向 `core/wcm/components/page/v2/page`. 此屬性允許WKND的頁面元件繼承 **all** 核心元件頁面元件功能的區段。 這是第一個 [代理元件模式](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/developing/guidelines.html#ProxyComponentPattern). 如需詳細資訊，請參閱 [此處](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/developing/guidelines.html).

1. Inspect WKND元件中的另一個元件，即 `Breadcrumb` 元件位於： `/apps/wknd/components/breadcrumb`. 請注意 `sling:resourceSuperType` 屬性可以找到，但這次它指向 `core/wcm/components/breadcrumb/v2/breadcrumb`. 這是使用Proxy元件模式來包含核心元件的另一個範例。 事實上，WKND程式碼基底中的所有元件都是AEM核心元件的代理（我們著名的HelloWorld元件除外）。 最佳實務是盡量嘗試和重複使用核心元件的功能 *befor* 撰寫自訂程式碼。
...
1. 下一步檢查核心元件頁面： `/libs/core/wcm/components/page/v2/page` 使用CRXDE Lite:
   ...
   請注意，此頁面下方還包含更多指令碼。 核心元件頁面包含許多功能。 此功能已細分為多個指令碼，以方便維護及閱讀。 您可以借由開啟 `page.html` 和尋找 `data-sly-include`:
...
目前，我們將像處理其他程式碼一樣處理範本，並同步 **文章頁面範本** 作為項目的一部分。 直到現在 **推送** 代碼從我們的AEM專案傳送至AEM的本機例項。 此 **文章頁面範本** 是直接在本機的AEM例項上建立，因此，我們 **匯入** 範本放入我們的AEM專案。 此 **ui.content** 模組會包含在AEM專案中，以用於此特定用途。

接下來的幾個步驟將使用VSCode IDE進行，使用 [VSCode AEM同步](https://marketplace.visualstudio.com/items?itemName=yamato-ltd.vscode-aem-sync&amp;ssr=false#overview) 插件，但可以使用您配置為 **匯入** 或從本機AEM例項匯入內容。
...
1. 重複匯入內容的步驟，但選取 **原則** 位於 `/conf/wknd/settings/wcm/policies`.
...
1. Inspect `filter.xml` 位於 `ui.content/src/main/content/META-INF/vault/filter.xml`.
   ...
   >[!WARNING]
   >
   > 為了確保WKND參考網站的一致部署，已設定專案的某些分支，使 `ui.content` 覆寫JCR中的任何變更。 這是根據設計（即解決方案分支），因為程式碼/樣式是針對特定原則所編寫。
...
此時，文章頁面的樣式明顯不同。 關注 [用戶端程式庫和前端工作流程](client-side-libraries.md) 教學課程，了解如何納入CSS和Javascript以將全域樣式套用至網站，以及整合專用的前端組建的最佳實務。

在上檢視完成的程式碼 [GitHub](https://github.com/adobe/aem-guides-wknd) 或是在本機的Git程式庫上檢閱並部署程式碼 `tutorial/pages-templates-solution`.
