---
title: 建立自訂元件 | AEM SPA編輯器快速入門與回應
...
您隨時都可以在 [GitHub上檢視完成的程式碼](https://github.com/adobe/aem-guides-wknd-spa/tree/React/custom-component-solution) ，或切換至分支，在本機檢出程式碼 `React/custom-component-solution`。
...
2. 導覽至 `ui.apps/src/main/content/jcr_root/apps/wknd-spa-react/components` 並建立名為的新資料夾 `custom-component`。
...
2. 在以下位置開啟Java `CustomComponent.java` 介面 `core/src/main/java/com/adobe/aem/guides/wknd/spa/react/core/models/CustomComponent.java`:
...
3. 更新 `CustomComponent.java` 以擴展界 `ComponentExporter` 面：
   ...
   該 `CustomComponent` 介面包括單個getter方法 `getMessage()`。 這是透過JSON模型公開作者對話方塊值的方法。 JSON模型中只會匯出含空 `()` 參數的公用getter方法。

4. 開啟 `CustomComponentImpl.java` 位置 `core/src/main/java/com/adobe/aem/guides/wknd/spa/react/core/models/impl/CustomComponentImpl.java`。

   這是介面的實 `CustomComponent` 施。 注 `@Model` 釋將Java類識別為Sling Model。 注 `@Exporter` 釋可讓Java類別透過Sling Model Exporter序列化和匯出。

5. 更新靜態變 `RESOURCE_TYPE` 數，以指向在上一個練習 `wknd-spa-react/components/custom-component` 中建立的AEM元件。

   ```java
   static final String RESOURCE_TYPE = "wknd-spa-react/components/custom-component";
   ```

   元件的資源類型是將Sling Model系結至AEM元件，最終會對應至React元件的內容。
...
6. 將方法 `getExportedType()` 添加到類中 `CustomComponentImpl` 以返回元件資源類型：
   ...
   在實施介面時需要此方 `ComponentExporter` 法，並將公開允許映射到React元件的資源類型。
...
7. 更新方 `getMessage()` 法以傳回作者對話 `message` 方塊所保留的屬性值。 使用注 `@ValueMap` 釋將JCR值映射 `message` 到Java變數：
   ...
   新增一些額外的「商業邏輯」，以傳回含所有大寫字母之訊息的字串值。 這可讓我們查看作者對話方塊儲存的原始值與Sling Model公開的值之間的差異。

   >[!NOTE]
   >
   > 您可以在此處檢 [視完成的CustomComponentImpl.java](https://github.com/adobe/aem-guides-wknd-spa/blob/React/custom-component-solution/core/src/main/java/com/adobe/aem/guides/wknd/spa/react/core/models/impl/CustomComponentImpl.java)。

## 更新React元件

自訂元件的React程式碼已建立。 接下來，進行一些更新，將Rearcet元件對應至AEM元件。

1. 在模 `ui.frontend` 塊中開啟檔案 `ui.frontend/src/components/Custom/Custom.js`。
2. 在方 `{this.props.message}` 法中觀察變 `render()` 數：
   ...
   預期Sling Model中轉換的大寫值將會對應至此屬 `message` 性。
...
4. 使用您的Maven技能，從專案目錄的根目錄將所有更新部署至本機AEM環境：
...
1. 導覽至http://localhost:4502/system/console/status-slingmodels以驗證新Sling Model的註冊 [情況](http://localhost:4502/system/console/status-slingmodels)。
   ...
   您應該會看到上述兩行，指出 `CustomComponentImpl` 此元件與元件相 `wknd-spa-react/components/custom-component` 關聯，且其已透過Sling Model Exporter註冊。
...
5. 導覽至http://localhost:4502/content/wknd-spa-react/us/en.model.json以檢視JSON模 [型](http://localhost:4502/content/wknd-spa-react/us/en.model.json)。 搜尋 `wknd-spa-react/components/custom-component`:
...
恭喜您，您已學習如何建立自訂AEM元件，以便與SPA編輯器搭配使用。 您也瞭解對話方塊、JCR屬性和Sling Models如何互動以輸出JSON模型。

您可以在 [GitHub上檢視完成的程式碼](https://github.com/adobe/aem-guides-wknd-spa/tree/React/custom-component-solution) ，或切換至分支，在本機檢出程式碼 `React/custom-component-solution`。
