...
本章會將簡單的`Header`元件新增至SPA。 In the process of building out this **static** `Header` component several approaches to AEM SPA development are used.
...
檢閱設定[本機開發環境](overview.md#local-dev-environment)所需的工具與指示。 This chapter is a continuation of the [Create Project](create-project.md) chapter, however to follow along all you need is a working SPA-enabled AEM project.
...
1. In the IDE of your choice open your AEM Project. 此教學課程將使用[Visual Studio Code IDE](https://experienceleague.adobe.com/docs/experience-manager-learn/cloud-service/local-development-environment-set-up/development-tools.html?lang=zh-Hant#microsoft-visual-studio-code)。
...
1. 導覽至 `ui.apps` 檔案夾。 在`ui.apps/src/main/content/jcr_root/apps/wknd-spa-react/clientlibs/clientlib-react`下方，您應該會看到已從`ui.frontend/build`資料夾複製編譯的SPA檔案。
...
1. 開啟新的瀏覽器索引標籤（如果尚未開啟），並導覽至[http://localhost:3000/content/wknd-spa-react/us/en/home.html](http://localhost:3000/content/wknd-spa-react/us/en/home.html)。
   ...
   >[!NOTE]
   >
   > 由於AEM的安全性需求，您必須在相同瀏覽器中（但在其他索引標籤中）登入本機AEM執行個體(http://localhost:4502)。
...
1. 返回位於[http://localhost:3000/content/wknd-spa-react/us/en/home.html](http://localhost:3000/content/wknd-spa-react/us/en/home.html)的瀏覽器。 您應該會立即看到應用程式的變更反映出來。
...
2. 導覽至[http://localhost:4502/editor.html/content/wknd-spa-react/us/en/home.html](http://localhost:4502/editor.html/content/wknd-spa-react/us/en/home.html)。 You should see the updated `Header` and styles applied.
...
恭喜，您已更新SPA並探索與AEM整合！ You now know how to develop the SPA against the AEM JSON model API using a **webpack-dev-server**.

### 後續步驟 {#next-steps}

[Map SPA components to AEM components](map-components.md) - Learn how to map React components to Adobe Experience Manager (AEM) components with the AEM SPA Editor JS SDK. 元件對應可讓使用者在AEM SPA Editor中對SPA元件進行動態更新，類似於傳統的AEM編寫。
...
1. 在`ui.frontend/public/index.html`開啟檔案`index.html`。 Update the metadata property for the AEM page model to point to a variable `%REACT_APP_PAGE_MODEL_PATH%`:
...
1. 如果目前正在執行，請停止&#x200B;**webpack-dev-server**。 從終端機啟動&#x200B;**webpack-dev-server**：
   ...
   導覽至[http://localhost:3000/content/wknd-spa-react/us/en/home.html](http://localhost:3000/content/wknd-spa-react/us/en/home.html)，您應該會看見內容與&#x200B;**proxy** json中所使用內容相同的SPA。
