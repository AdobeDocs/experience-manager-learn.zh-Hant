...
本章檢查如何提供 `Text` 組SPA件映射到AEM `Text`元件。 響應核心元件，如 `Image` 組SPA件在中使SPA用並創作AEM。 開箱即用功能 **佈局容器** 和 **模板編輯器** 策略還用於建立外觀稍有變化的視圖。
...
1. 下一個，開啟 `ui.frontend/src/components/import-components.js` 看看 `TextEditConfig` 在~86號線：
