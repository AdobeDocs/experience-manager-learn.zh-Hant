...
本章將檢查如何提供 `Text` 組SPA件映射到AEM `Text`元件。 新 `Image` 將創SPA建可在中使用SPA和創作的組AEM件 開箱即用功能 **佈局容器** 和 **模板編輯器** 策略還將用於建立外觀更多樣的視圖。
...
8. Inspect **text.component.html** 在 `ui.frontend/src/app/components/text/text.component.html`。
   ...
   此檔案為空，因為元件的整個內容將由 `innerHTML` 屬性。
...
2. 返回到IDE並開啟 `en.model.json` 在 `ui.frontend/src/mocks/json/en.model.json`。 由於這是我們項目的一個新元件，因此我們需要「模擬」映像JSON。
   ...
   該項目包括位於 `/mock-content/adobestock-140634652.jpeg` 將用於 **WebPack Dev伺服器**。
