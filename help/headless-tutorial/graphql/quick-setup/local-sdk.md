...
AEM無頭式快速設定可讓您使用WKND Site範例專案中的內容，以及透過AEM無頭式GraphQL API取用內容的範例React應用程式(SPA)，來與AEM無頭式實作。 本指南使用 [AEMas a Cloud ServiceSDK](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/implementing/developing/aem-as-a-cloud-service-sdk.html?lang=en#aem-as-a-cloud-service-sdk).
...
## 2.安裝範例WKND內容 {#wknd-site-content}
...
1. 在您選擇的IDE中開啟檔案 `.env.development` at `aem-guides-wknd-graphql/react-app/.env.development`. 確認 `REACT_APP_AUTHORIZATION` 行未加註，檔案如下所示：
   ...
   >[!NOTE]
   >
   > 在生產案例中，應用程式會連線至AEM **發佈** 環境。 這在 [生產部署](../multi-step/production-deployment.md) 章節。
...
1. 按一下 **上傳套件** 並選擇在上一步下載的包。 按一下 **安裝** 安裝套件。
   ...
   ![安裝GraphiQL包](../multi-step/assets/explore-graphql-api/install-graphiql-package.png)
1. 導航到GraphiQL IDE，位置為 [http://localhost:4502/content/graphiql.html](http://localhost:4502/content/graphiql.html) 並開始探索GraphQL API。
   ...
   >[!NOTE]
   >
   > GraphiQL工具和GraphQL API是 [稍後在教學課程中詳細探討](../multi-step/explore-graphql-api.md).
