...
description: 無AEM頭快速設定可讓您使用WKND站點示例項目中的內AEM容與無頭進行操作，並使用通過無頭GraphQLAPI使用內容的反AEM應應用。
...
無AEM頭快速設定可讓您使用WKND站點示例項目中的內容與無頭實AEM際操作，以及一個示例反應應用(aSPA)，該應用會通過無頭GraphQLAPIAEM消耗內容。
...
請記住，WKND站點項目提供了React應用程式通過無頭GraphQLAPIAEM使用的示例內容。
...
通AEM過WKND站點項目中的內容as a Cloud Service引導，下載並啟動WKND React App示例，該示例通過無頭GraphQLAPI消耗WKND站點的AEM內容。
...
1. 本地運行的React App啟動 [http://localhost:3000](http://localhost:3000) 並顯示一份冒險記錄，這些記錄來自AEM使用Headless公司的AEMGraphQLAPI的as a Cloud Service。

## 5.編輯內AEM容

WKND React App示例連接到無頭GraphQLAPI並從其中消耗內容AEM，在AEM Author服務中編寫內容，並查看React App的體驗如何一致更新。
