---
title: 作者內容片段 — 無頭的高級AEM概念 — GraphQL
...
本文檔是多部分教程的一部分。 請確保 [上一章](create-content-fragment-models.md) 在繼續本章之前已完成。
...
1. 下載 [高級 — GraphQL — 教程 — 簡易版 — 軟體包–1.1.zip](/help/headless-tutorial/graphql/advanced-graphql/assets/tutorial-files/Advanced-GraphQL-Tutorial-Starter-Package-1.1.zip)
...
從主AEM頁選擇 **資產** > **檔案** > **WKND共用** > **英語**。 這裡您可以看到各種內容片段類別，包括冒險和參與者。
...
1. 選擇 **策略** ，然後取消選擇 **繼承自/content/dam/wknd-shared**。 在 **允許的內容片段模型（按路徑）** 的子菜單。

   ![資料夾表徵圖](assets/author-content-fragments/folder-icon.png)

1. 在開啟的「選擇路徑」對話框中，沿路徑 **會議** > **WKND共用**。 在上一章中建立的「人員內容片段模型」包含對「聯繫資訊內容片段模型」的引用。 必須允許「教師」資料夾中的「人員」和「聯繫人資訊」模型，才能建立「教師內容」部分。 選擇 **人員** 和 **聯繫資訊**&#x200B;按 **選擇** 按鈕
...
按照上一節中概述的步驟 [建立資料夾](#create-folders) 建立標題為「聯繫人資訊」的資料夾。 嵌套資料夾繼承父資料夾的資料夾策略。 您可以隨意配置更具體的策略，以便新建立的資料夾僅允許使用「聯繫資訊」模型。

### 建立教師內容片段

讓我們建立四個可以添加到冒險指導團隊的人員。

1. 從「指導者」資料夾中，基於「人員內容片段模型」建立內容片段，並為其指定「Jacob Wester」的標題。
...
1. 在欄位中輸入以下內容：
   ...
   * **全名**:雅各布·韋斯特
   * **傳記**:雅各布·韋斯特是徒步教練已經十年了，每分每秒都很喜歡！ 雅各布是個探險探索者，擅長攀岩和背包。 雅各布是攀岩比賽的贏家，包括海灣巨石競賽。 雅各布目前住在加利福尼亞。
   * **教師經驗級別**:專家
   * **技能**:攀岩、衝浪、背包
   * **管理員詳細資訊**:雅各布·韋斯特已經協調背包冒險了三年。

1. 在 **配置檔案圖片** 欄位中，將內容引用添加到影像。 瀏覽到 **WKND共用** > **英語** > **撰稿人** > **雅各布·韋斯特·jpg** 來修改標籤元素的屬性。

### 從內容片段編輯器建立片段引用 {#fragment-reference-from-editor}
...
| 內容片段標題 | 史黛絲·羅斯威爾斯 |
| 完整名稱 | 史黛絲·羅斯威爾斯 |
| 聯絡資訊 | /content/dam/wknd shared/en/adventures/autintors/contact-info/stacey-roswells-contact-info |
| 配置檔案圖片 | /content/dam/wknd-shared/en/contributors/stacey-roswells.jpg |
| 傳記 | 史黛西·羅斯威爾斯是一位非常出色的攀岩者和阿爾卑斯山探險家。 史黛絲出生在馬里蘭州的巴爾的摩，是六個孩子中最小的。 史黛絲的父親是美國海軍的一名中校，母親是現代舞教官。 史黛絲的家人經常隨父親的任務搬家，父親在泰國駐紮時拍了第一張照片。 史黛絲也是在這裡學會攀岩的。 |
| 教師經驗級別 | 進階 |
| 技能 | 攀岩 |滑雪 |回裝 |
...
| 內容片段標題 | 庫馬爾·塞爾瓦拉伊 |
| 完整名稱 | 庫馬爾·塞爾瓦拉伊 |
| 聯絡資訊 | /content/dam/wknd shared/en/adventures/attintors/contact-info/kumar-selvaraj-contact-info |
| 配置檔案圖片 | /content/dam/wknd-shared/en/contributors/kumar-selvaraj.jpg |
...
| 內容片段標題 | 奧貢森德 |
| 完整名稱 | 奧貢森德 |
| 聯絡資訊 | /content/dam/wknd共用/en/adventures/attiners/contact-info/ayo-ogunseinde contact-info |
| 配置檔案圖片 | /content/dam/wknd-shared/en/contributors/ayo-ogunseinde-237739.jpg |
| 傳記 | Ayo Ogunseinde是一位職業登山者和背包教練，住在加州中部的弗雷史諾。 阿約的目標是引導徒步者進行他們史詩般的國家公園探險。 |
| 教師經驗級別 | 進階 |
| 技能 | 攀岩 |循環 |回裝 |
...
暫時忽略Yosemite Valley Lodge資料夾。 我們稍後在本節中返回它，當我們建立一個位置作為我們的教師團隊的家庭基礎時。

導航到 **約塞米蒂國家公園** 的子菜單。 目前，它只包含約塞米蒂國家公園的照片。 讓我們使用位置內容片段模型建立內容片段，並將其命名為「Yosemite National Park」。
...
讓我們將其餘內容添加到位置內容片段，以便在下一章向GraphQL查詢資訊。
...
1. 從 **聯繫資訊** 欄位中，根據「聯繫資訊」模型建立內容片段，並將其標題為「Yosemite National Park Contact Info」。 遵循上一節中概述的相同流程。 [從編輯器建立片段引用](#fragment-reference-from-editor) 並在欄位中輸入以下資料：
...
1. 從 **位置影像** 欄位，瀏覽 **冒險** > **位置** > **約塞米蒂國家公園** > **約塞米蒂國家公園.jpeg** 來修改標籤元素的屬性。
   ...
   請記住，在上一章中，您配置了映像驗證，因此「位置」映像的尺寸必須小於2560 x 1800，其檔案大小必須小於3 MB。

1. 添加所有資訊後， **位置詳細資訊** 頁籤現在如下所示：

   ![「位置詳細資訊」頁籤已完成](assets/author-content-fragments/location-details-tab-completed.png)

1. 導航到 **位置地址** 頁籤。 從 **地址** 欄位中，使用您在上一章中建立的地址內容片段模型建立標題為「Yosemite National Park Address」的內容片段。 按照上一節中概述的相同流程 [從編輯器建立片段引用](#fragment-reference-from-editor) 並在欄位中輸入以下資料：
...
1. 導航到 **約塞米蒂谷旅館** 的子菜單。 使用「位置內容片段模型」建立內容片段，並將其命名為「Yosemite Valley Lodge」。
...
1. 從 **聯繫資訊** 欄位中，根據「聯繫資訊」模型建立內容片段，並將其標題為「Yosemite Valley Lodge Contact Info」。 按照上一節中概述的相同流程 [從編輯器建立片段引用](#fragment-reference-from-editor) 並在新內容片段的欄位中輸入以下資料：
...
1. 導航到 **約塞米蒂谷旅館** 去 **位置地址** 頁籤。 從 **地址** 欄位中，使用您在上一章中建立的地址內容片段模型建立標題為「Yosemite Valley Lodge Address」的內容片段。 按照上一節中概述的相同流程 [從編輯器建立片段引用](#fragment-reference-from-editor) 並在欄位中輸入以下資料：
...
1. 瀏覽到 **WKND共用** > **英語** > **冒險** > **位置** > **約塞米蒂谷旅館** > **約塞米蒂谷旅館**。 按 **選擇** 的子菜單。
...
1. 在顯示的新欄位中，選擇資料夾表徵圖以開啟「選擇路徑」模式。 瀏覽資料夾到 **WKND共用** > **英語** > **冒險** > **指導員**，然後選擇旁邊的複選框 **雅各·韋斯特**。 按 **選擇** 的子菜單。
...
1. 在中添加片段引用 **位置** 的子菜單。 位置路徑應引用您建立的Yosemite National Park內容片段： `/content/dam/wknd-shared/en/adventures/locations/yosemite-national-park/yosemite-national-park`。

1. 在中添加片段引用 **教師團隊** 的子菜單。 團隊路徑應引用您建立的Yosemite團隊內容片段： `/content/dam/wknd-shared/en/adventures/teams/yosemite-team/yosemite-team`。 這是嵌套片段引用。 團隊內容片段包含引用聯繫資訊和地址模型的人員模型的引用。 因此，您將嵌套內容分段向下分為三個級別。

1. 在中添加片段引用 **管理員** 的子菜單。 假設雅各布·韋斯特是優勝美地背包探險的管理員。 路徑應指向Jacob Wester內容片段，如下所示： `/content/dam/wknd-shared/en/adventures/instructors/jacob-wester`。
...
在 [下一章](/help/headless-tutorial/graphql/advanced-graphql/explore-graphql-api.md)，瞭解如何使用GraphiQL整合開發環境(IDE)發送高級GraphQL查詢。 這些查詢允許我們查看本章中建立的資料，並最終將這些查詢添加到WKND應用。
