---
title: 建立內容片段模型 — 無頭的高級概AEM念 — GraphQL
...
這是高級教程。 在繼續本章之前，請確保您已完成 [快速設定](../quick-setup/cloud-service.md)。 確保您還閱讀了上一個概述章節，瞭解有關高級教程設定的詳細資訊。
...
2. 為模型提供「聯繫資訊」標題，然後選擇 **建立**。 在顯示的成功模式中，選擇 **開啟** 的子菜單。

3. 從拖動 **單行文本** 的子菜單。 給它 **欄位標籤** 在 **屬性** 頁籤。 屬性名稱自動填寫為 `phone`。 選中複選框以建立欄位 **必需**。

4. 導航到 **資料類型** 頁籤，然後添加另一個 **單行文本** 的子菜單。 給它 **欄位標籤** 並將其設定為 **必需**。
...
1. 從 **WKND站點**&#x200B;選中 **建立** 從右上角。

2. 輸入「地址」標題，然後選擇 **建立**。

   我。在顯示的成功模式中，選擇 **開啟** 的子菜單。

3. 拖放 **單行文本** 在模型上給它 **欄位標籤** 街道地址。 然後，屬性名稱將填充為 `streetAddress`。 選擇 **必需** 複選框。

4. 重複上述步驟，並向模型添加四個「單行文本」欄位。 使用以下標籤：
...
2. 為模型指定「人員」標題，然後選擇 **建立**。 在顯示的成功模式中，選擇 **開啟** 的子菜單。

3. 從拖動 **單行文本** 的子菜單。 給它 **欄位標籤** 全名。 屬性名稱自動填寫為 `fullName`。 選中複選框以建立欄位 **必需**。

   ![全名選項](assets/define-content-fragment-models/full-name.png)

4. 內容片段模型可以在其他模型中引用。 導航到 **資料類型** 頁籤，然後拖放 **片段引用** 的子菜單。

5. 在 **屬性** 頁籤 **允許的內容片段模型** 的子菜單。 **聯繫資訊** 早先建立的片段模型。

6. 添加 **內容引用** 並給它 **欄位標籤** 「配置檔案圖片」。 選擇下面的資料夾表徵圖 **根路徑** 的子菜單。 通過選擇 **內容** > **資產**，然後選中的複選框 **WKND站點**。 使用 **選擇** 按鈕。 最後文本路徑應為 `/content/dam/wknd`。

   ![內容引用根路徑](assets/define-content-fragment-models/content-reference-root-path.png)

7. 下 **僅接受指定的內容類型**，選擇「影像」。

   ![配置檔案圖片選項](assets/define-content-fragment-models/profile-picture.png)

8. 要限制影像檔案大小和尺寸，讓我們查看內容引用欄位的一些驗證選項。

   下 **僅接受指定的檔案大小**，選擇「小於或等於」，下面將顯示其他欄位。
   ![僅接受指定的檔案大小](assets/define-content-fragment-models/accept-specified-file-size.png)

9. 對於 **最大**，輸入&quot;5&quot;和 **選擇設備**，選擇「兆位元組(MB)」。 此驗證僅允許選擇具有指定大小的影像。
...
2. 添加 **頁籤佔位符** 欄位，並將其標籤為「位置詳細資訊」。

3. 拖放 **單行文本** 並標上&quot;名&quot; 在此欄位標籤下，添加 **多行文本** 欄位並將其標籤為「說明」。

4. 下一步，添加 **片段引用** 欄位，並將其標籤為「聯繫資訊」。 在「屬性」頁籤中，在 **允許的內容片段模型**，選擇 **資料夾表徵圖** 並選擇先前建立的「聯繫資訊」片段模型。

5. 添加 **內容引用** 欄位。 將其標籤為「位置映像」。 的 **根路徑** 應該 `/content/dam/wknd.` 下 **僅接受指定的內容類型**，選擇「影像」。

6. 我們也添加 **JSON對象** 的子菜單。 由於此資料類型靈活，因此可用於顯示要包括在內容中的任何資料。 在這種情況下，JSON對象用於顯示有關天氣的資訊。 將JSON對象標籤為「Weather by Season」。 在 **屬性** 頁籤，添加 **說明** 因此，用戶可以清楚地知道應在此處輸入哪些資料：&quot;有關按季節（春、夏、秋、冬）的活動地點天氣的JSON資料。&quot;

   ![JSON對象選項](assets/define-content-fragment-models/json-object.png)

7. 要建立「位置地址」頁籤，請添加 **頁籤佔位符** 欄位，並將其標籤為「位置地址」。

8. 拖放 **片段引用** 欄位和屬性頁籤中，位於 **允許的內容片段模型**，選擇 **地址** 模型。

9. 選擇 **保存** 確認更改並關閉內容片段模型編輯器。 已完成的位置模型應顯示如下：
...
[無AEM頭和GraphQL視頻系列](/help/headless-tutorial/graphql/video-series/author-publish-architecture.md)。
