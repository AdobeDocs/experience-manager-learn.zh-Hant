---
title: 使用AEM Publish服務進行生產部署——無頭入門AEM- GraphQL
description: 瞭解AEM Author和Publish服務，以及建議的無頭應用程式部署模式。 在本教程中，學習如何使用環境變數根據目標環境動態更改GraphQL端點。 瞭解如何正確AEM設定跨來源資源共用(CORS)。
sub-product: 資產
topics: headless
...
在本教學課程中，您將設定本機環境，以模擬內容從「作者」例項散發至「發佈」例項。 您也會使用GraphQL API，產生React App的生產組建版本，此應用程式設定為使用AEM Publish環境中的內容。 在此過程中，您將學習如何有效使用環境變數，以及如何更新AEMCORS組態。
...
* 瞭解管理環境變數的最佳做法。
* 瞭解如何正確設AEM定跨原始碼資源共用(CORS)。

## 作者發佈部署模式{#deployment-pattern}

完整AEM的環境由Author、Publish和Dispatcher組成。 「作者」服務是內部使用者建立、管理和預覽內容的地方。 「發佈」服務被視為「即時」環境，一般是使用者與之互動的環境。 內容在「作者」服務上經過編輯和核准後，會分發至「發佈」服務。
...
1. 當實AEM例完成安裝後，將在[http://localhost:4503/content.html](http://localhost:4503/content.html)開啟新的瀏覽器窗口

   預期會傳回404找不到頁面。 這是全新的例AEM項，尚未安裝任何內容。
...
1. 開啟新瀏覽器並導覽至[http://localhost:5000/](http://localhost:5000/)。 您應該會看到React App正在提供。
   ...
   請注意，GraphQL查詢正在首頁上工作。 Inspect使用您的開發人員工具，將&#x200B;**XHR**&#x200B;要求轉換為「XHR」。 請注意，GraphQLPOST是位於`http://localhost:4503/content/graphql/global/endpoint.json`的Publish實例。
...
由於`<img src`屬性被設定為相對路徑，最終指向`http://localhost:5000/`處的Node靜態伺服器，因此映像會出現中斷。 這些影像應該會指向AEM Publish實例。 這有幾種可能的解決辦法。 使用webpack dev伺服器時，檔案`react-app/src/setupProxy.js`在webpack伺服器和作者實例之間為對&lt;a1/AEM>的任何請求設定代理。 `/content`代理配置可用於生產環境，但必須在Web伺服器級別配置。 例如，[Apache的代理模組](https://httpd.apache.org/docs/2.4/mod/mod_proxy.html)。

應用程式可以更新為包含使用`REACT_APP_HOST_URI`環境變數的絕對URL。 我們改用GraphQL API的功AEM能來請求影像的絕對URL。
...
1. 下載軟體包[EnableReplicationAgent.zip](./assets/publish-deployment/EnableReplicationAgent.zip)，然後使用軟體包管理器進行安裝。
   ...
   >[!NOTE]
   >
   > 在作AEM為Cloud Service環境時，作者層會自動設定為將內容發佈至發佈層。

1. 從&#x200B;**開始AEM**&#x200B;功能表，導覽至&#x200B;**工具** > **資產** > **內容片段模型**。
...
預設AEM為安全，不允許非AEMWeb屬性進行用戶端呼叫。 跨AEM原始資源共用(CORS)設定可讓特定網域呼叫AEM。
...
1. 搜尋&#x200B;**Apache Sling Referrer Filter**&#x200B;並檢視設定。 還需要&#x200B;**允許空**&#x200B;配置，才能啟用來自外部域的GraphQL請求。
   ...
   >[!NOTE]
   >
   > OSGi配置在承諾進行源AEM控制的項目中進行管理。 您可AEM以使用Cloud Manager將專案部AEM署為Cloud Service環境。 [項目原AEM型](https://github.com/adobe/aem-project-archetype)可協助產生專案以供特定實施。
...
恭喜！ 您現在已使用AEM Publish環境模擬完整的生產部署。 您也已在中學習如何使用CORS設定AEM。
...
* [與內AEM容片段搭配使用的GraphQL API](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/assets/admin/graphql-api-content-fragments.html)
...
* [將程式碼部AEM署為Cloud Service](https://experienceleague.adobe.com/docs/experience-manager-learn/cloud-service/cloud-manager/devops/deploy-code.html?lang=en#cloud-manager)
