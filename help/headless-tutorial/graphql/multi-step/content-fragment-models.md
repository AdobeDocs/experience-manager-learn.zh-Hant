...
在本章中，了解如何建立內容模型，以及使用 **內容片段模型**. 您將了解可用來定義模型中結構的不同資料類型。

在本章中建立了兩個簡單模型， **團隊** 和 **人員**. 此 **團隊** 資料模型的名稱、短名稱和說明，並且引用 **人員** 資料模型，有全名、簡歷細節、簡介圖片和職業清單。
...
專案設定包含與特定專案相關聯的所有內容片段模型，並提供組織模型的方法。 至少必須建立一個項目 **befor** 建立新內容片段模型。
...
1. 在產生的對話方塊中輸入：
   ...
   * 標題*: **我的專案**
   * 名稱*: **my-project** (使用連字型大小來分隔字詞時，請使用全部小寫。 此字串將影響客戶端應用程式將針對的唯一GraphQL端點執行請求。)
   * 檢查 **內容片段模型**
   * 檢查 **GraphQL持久查詢**
...
1. 按一下 **資料類型** 標籤並拖放 **內容參考** 欄位。 輸入以下屬性：
   ...
   * **根路徑**: `/content/dam`
   ...
   設定 **根路徑** 您可以按一下 **資料夾** 圖示來開啟強制回應視窗以選取路徑。 這將限製作者可用來填入路徑的資料夾。 `/content/dam` 是儲存所有AEM資產（影像、視訊、其他內容片段）的根目錄。
...
為 **團隊**，即一組人員的資料模型。 「團隊」模型將參考「人員」模型來表示團隊的成員。

1. 在 **我的專案** 資料夾，點選 **建立** 在右上角 **建立模型** 嚮導。
1. 針對 **模型標題** 輸入： **團隊** 點選 **建立**.
...
1. 點選 **資料類型** 標籤並拖放 **單行文字** 元素。 在 **屬性** 標籤：
   ...
   * 檢查 **不重複**
   * 在 **驗證類型** >選擇 **自訂**
   * 在 **自訂驗證Regex** >輸入 `^[a-z0-9\-_]{5,40}$`  — 這將確保只能輸入小寫的英數字元值，並且5到40個字元之間的破折號。
...
在下一章中， [製作內容片段模型](author-content-fragments.md)，您將根據內容片段模型建立和編輯新內容片段。 您也會學習如何建立內容片段的變體。
