...
Dispatcher是大部分的必要元件，如果不是所有AEM安裝的話。 您可以找到許多線上文章，討論如何設定Dispatcher以及提示和訣竅。

不過，這些零星的資訊總是從非常技術性的層面開始 — 假設您已知道要做什麼，因此只提供如何達成所要目標的詳細資訊。 我們從未找到任何概念性檔案來說明 _什麼是，為什麼是_ 決定您可以和無法對Dispatcher執行的操作。

### 反模式：Dispatcher作為事後思考

缺乏基本資訊會導致許多反模式，我們在許多AEM專案中看到：

1. 由於Dispatcher已安裝在Apache Web伺服器上，因此必須由專案中的「Unix gods」負責對其進行設定。 「凡世的Java開發人員」不需要擔心這個問題。

2. Java開發人員需要確保他的程式碼可以運作…… Dispatcher稍後會神奇地使其快速。 Dispatcher一律為事後考量。 但是，這無法運作。 開發人員在設計程式碼時必須考慮到Dispatcher。 他需要瞭解它的基本概念才能做到這一點。

### 「先讓它運作，再讓它快速」並非總是正確的

您可能聽過程式設計的建議 _「先讓它運作，然後再讓它快速。」_。這並非完全錯誤。 但是，如果沒有正確的上下文，它往往會遭到錯誤解譯且無法正確套用。
...
Dispatcher不會奇蹟般地提高程式碼的速度。 開發人員建立元件時，必須考慮到Dispatcher。 因此，他需要瞭解運作方式。
...
在網路早期，您可能會有幾百位訪客造訪網站。 一個Dispatcher的設定，「已分派」或平衡許多AEM發佈伺服器的請求負載，通常情況下這已足夠 — 因此，名稱為「Dispatcher」。 然而，現今此設定不再經常使用。
...
1. 使用者請求頁面
2. Dispatcher會檢查它是否已擁有該頁面的轉譯版本。 假設這是此頁面的第一個請求，且Dispatcher找不到本機快取副本。
3. Dispatcher會從發佈系統請求頁面
4. 在發佈系統上，頁面會由JSP或HTL範本轉譯
5. 頁面會傳回Dispatcher
6. Dispatcher快取頁面
7. Dispatcher會將頁面傳回給瀏覽器
8. 如果第二次請求相同頁面，則可直接從Dispatcher快取中提供該頁面，而不需要在Publish執行個體上重新轉譯。 這樣可節省使用者和CPU在發佈執行個體上的等待時間。

我們在最後一節討論「頁面」。 但相同的配置也適用於其他資源，例如影像、CSS檔案、PDF下載等。

#### 如何快取資料

Dispatcher模組會利用託管Apache伺服器提供的設施。 HTML頁面、下載和圖片等資源會儲存為Apache檔案系統中的簡單檔案。 就是這麼簡單。

檔案名稱是由要求的資源URL衍生。 如果您要求檔案 `/foo/bar.html` 例如會儲存在/底下`var/cache/docroot/foo/bar.html`.

原則上，如果所有檔案都快取下來，然後靜態儲存在Dispatcher中，您可以提取Publish系統的外掛程式，而Dispatcher就當作簡單的網頁伺服器。 但這只是為了說明原則。 現實生活更複雜。 您不能快取所有內容，而且快取永遠不會完全「滿」，因為由於轉譯過程的動態性質，資源數量可能是無限的。 靜態檔案系統的模型有助於產生Dispatcher功能的粗略描述。 此外，它有助於說明Dispatcher的限制。
...
* `selectors` 其中，代表Sling中使用的許多選取器，以判斷如何呈現資源。 URL可以有任意數目的選取器。 它們以句點分隔。 例如，選取器區段可能類似於「french.mobile.fancy」。 選取器應僅包含字母、數字和破折號。

* `html` 「選取器」的最後一個稱為擴充功能。 在AEM/Sling中，它也部分決定轉譯指令碼。
...
* `#fragment`，URL的片段部分不會傳遞至AEM，而只會用於瀏覽器；在JavaScript架構中為「路由引數」，或跳至頁面上的特定部分。

在Apache (*請參考下圖*)，

* `pagename.selectors.html` 會作為快取檔案系統中的檔案名稱。

如果URL有尾碼 `path/suffix.ext` 然後
...
1. URL可能會變得很長。 新增的「路徑」部分 `/docroot` 在本機檔案系統上很容易超過某些檔案系統的限制。 在Windows上的NTFS中執行Dispatcher是一項挑戰。 不過，使用Linux是安全的。
...
3. 在CRX中，資源有子資源。 例如，一個頁面會有許多子頁面。 這在檔案系統中無法比對，因為檔案系統有檔案或資料夾。
...
在AEM中絕對有效。 您在本機開發電腦上不會看到任何問題（沒有Dispatcher）。 在UAT或負載測試中，您很可能也不會遇到任何問題。 我們面臨的問題是如此微妙，以至於在大部分測試中都未能通過。  當您處於尖峰時間，而且處理時間有限，可能沒有伺服器存取權，也沒有資源可加以修正時，就會受到嚴重影響。 我們曾經遇到過……

那麼……有什麼問題嗎？

`home.html` 在檔案系統中，可以是檔案或資料夾。 並非兩者都與AEM中的相同。

如果您要求 `home.html` 首先，會建立為檔案。

後續要求給 `home.html/suffix.html` 傳回有效的結果，但以檔案形式 `home.html` 「封鎖」檔案系統中的位置，  `home.html` 無法第二次建立為資料夾，因此 `home.html/suffix.html` 不會快取。
...
如果您反其道而行之，請先要求 `home.html/suffix.html` 則 `suffix.html` 已快取在資料夾下 `/home.html` 首先。 不過，此資料夾會被刪除並取代為檔案 `home.html` 當您後續要求時 `home.html` 作為資源。
...
因此，快取的結果完全是隨機的，取決於傳入請求的順序。 讓事情變得更棘手的是，您通常有多個排程程式。 而且效能、快取命中率和行為可能會因Dispatcher而有所不同。 如果您想瞭解網站無回應的原因，您必須確定您所檢視的Dispatcher是不幸的快取順序。 如果您檢視的Dispatcher幸運地擁有更有利的請求模式，您在嘗試尋找問題時將會失敗。
...
失效很簡單：對Dispatcher上的特殊「/invalidate」 URL發出的簡單GET請求。 不需要HTTP-body，「payload」只是「invalidate-path」標頭。 另請注意，標頭中的invalidate-path是AEM知道的資源，而非Dispatcher快取的檔案或檔案。 AEM只知道資源。 當要求資源時，會在執行階段使用擴充功能、選取器和尾碼。 AEM不會針對資源上已使用的選取器執行任何簿記，因此在啟用資源時，資源路徑是其知道的全部資訊。

在我們的案例中這已經足夠。 如果資源已變更，我們可以安全地假設，該資源的所有轉譯也已變更。 在我們的範例中，如果影像已變更，也會轉譯新的縮圖。

Dispatcher可以安全地刪除資源，及其已快取的所有轉譯。 它會執行類似以下的動作，
...
他發佈編輯過的「加拿大」頁面，並修訂先前發佈的首頁來檢視變更。 但是 — 沒有任何改變。 仍會顯示舊的Teaser。 他仔細檢視「冬季特惠」。 該頁面之前從未請求過，因此不會在Dispatcher中靜態快取。 因此，此頁面由Publish最新呈現，且此頁面現在包含新的「造訪加拿大」Teaser。
...
「冬季特殊優惠」頁面尚未轉譯，因此Dispatcher上沒有靜態版本，因此在提出要求時最新轉譯時會顯示新的Teaser。

您可能會認為，當資源變更時，Dispatcher在轉譯和清除所有曾使用此資源的頁面時會追蹤其觸及的每個資源。 但Dispatcher不會轉譯頁面。 轉譯是由發佈系統執行。 Dispatcher不知道哪些資源會進入轉譯的.html檔案。

還是不相信？ 您可能會認為 *「必須有方法可以實作某種相依性追蹤」*. 有，或者更準確地說 *為*. 公報3 AEM的曾曾祖父已於中實施相依性追蹤器 _session_ 用來呈現頁面的區段。
...
1. 在一般網站上，僅一小部分頁面會經常被要求。 因此，即使您捨棄所有演算後的內容，事後也只會要求幾十個內容。 呈現頁面長尾可在實際請求時隨時間分佈。 因此，實際上轉譯頁面的負載並不如您預期般高。 當然，總是會有例外……我們稍後會討論一些如何在具有空白Dispatcher快取的大型網站上處理平均分配負載的技巧。
...
![主要導覽透過提取頁面的「NavTitles」，不可避免地將所有頁面的內容結合在一起](assets/chapter-1/nav-titles.png)

*主要導覽透過提取頁面的「NavTitles」，不可避免地將所有頁面的內容結合在一起*
...
現在，如果您有一個包含數千個頁面的大型網站，則需要相當長的時間來循環瀏覽所有頁面並將它們實際刪除。 在此期間，Dispatcher可能會無意中提供過時內容。 更糟糕的是，存取快取檔案時可能會發生一些衝突，可能是某個頁面剛被刪除時即被請求，或該頁面在緊接著啟動後又發生第二次失效，因而再次被刪除。 想想會有多亂。 幸好事情不是這樣的。 Dispatcher會巧妙地避免此情況：它不會刪除數百個及數千個檔案，而是在檔案發佈時，將一個簡單、空白的檔案放入檔案系統的根目錄中，因此所有相依的檔案都被視為無效。 此檔案稱為「statfile」。 statfile是空的檔案 — 與statfile相關的是其建立日期。

Dispatcher中所有建立日期早於statfile的檔案，都已在上次啟動（及失效）前轉譯，因此視為「無效」。 它們實際上仍存在於檔案系統中，但Dispatcher會忽略它們。 它們「過時」。 每當發出過時資源的請求時，Dispatcher都會要求AEM系統重新轉譯頁面。 然後，新轉譯的頁面會儲存在檔案系統中 — 現在會有一個新的建立日期，而且會再次刷新。

![.stat檔案的建立日期會定義哪些內容已過時哪些內容是最新狀態](assets/chapter-1/creation-date.png)

*.stat檔案的建立日期會定義哪些內容已過時哪些內容是最新狀態*

<br> 

您可能會問，為何將其稱為「.stat」？ 也可能不是「.invalidated」？ 嗯，您可以想像，在檔案系統中擁有該檔案有助於Dispatcher判斷哪些資源可以 *靜態* 服務 — 就像從靜態Web伺服器一樣。 這些檔案不再需要動態呈現。

然而，這個名稱的真正本質並不那麼隱喻。 它衍生自Unix系統呼叫 `stat()`，會傳回檔案的修改時間（以及其他屬性）。
...
2. 如果您的代理程式傳送HTTP標頭 `CQ-Action-Scope: ResourceOnly`，表示此單一失效請求不會觸發自動失效。 這個( [https://github.com/cqsupport/webinar-dispatchercache/tree/master/src/refetching-flush-agent/refetch-bundle](https://github.com/cqsupport/webinar-dispatchercache/tree/master/src/refetching-flush-agent/refetch-bundle))程式碼片段可能是您自己的復寫代理程式的良好起點。

3. `ResourceOnly`，只會防止自動失效。 若要實際執行必要的相依性解析和無效化，您必須自行觸發無效化請求。 您可能需要檢查套件Dispatcher排清規則([https://adobe-consulting-services.github.io/acs-aem-commons/features/dispatcher-flush-rules/index.html](https://adobe-consulting-services.github.io/acs-aem-commons/features/dispatcher-flush-rules/index.html))，以獲得實際發生的靈感。

我們不建議您建立相依性解析配置。 工作太辛苦，卻收效甚微，而且如前所述，出錯的次數太多了。

而是應該找出哪些資源對其他資源沒有任何相依性，並且可以在沒有自動失效的情況下失效。 不過，您不需要就此使用自訂復寫代理程式。 只需在您的Dispatcher設定中建立自訂規則，從自動失效中排除這些資源。
...
在上一章，我們說明了Dispatcher的基本機制、其一般運作方式以及限制。
...
我們在這裡實作的方式是在許多專案中看到的常見模式，即使其中一個AEM核心元件也是以該模式為基礎。 因此，身為開發人員的您很可能調整該模式。 它在封裝方面有其優點，但需要付出大量努力才能使其為Dispatcher做好準備。 我們稍後會討論幾個如何緩解問題的選項。
...
元件有兩個部分；第一個部分呈現影像的HTML標籤，第二個部分則「卷軸」參照影像的二進位資料。 由於這是回應式設計的現代網站，因此呈現的並非簡單 `<img src"…">` 標籤，但在中的一組影像 `<picture/>` 標籤之間。 我們會為每個裝置上傳兩個不同的影像至DAM，並從影像元件中參照它們。
...
到Dispatcher。 不過，這些請求都是徒勞的。 內容已快取為元件子結構下的檔案。 這些檔案現已過時，但仍會應要求提供服務。
...
從AEM開發人員的角度來看，這個圖案看起來非常優雅。 但是，隨著Dispatcher進入方程式，您可能會同意，天真的方法可能不夠。
...
您有沒有見過類似以下的查詢引數 `?ck=398547283745`？ 這類檔案稱為cache-killer (&quot;ck&quot;)。 其想法是，如果您新增任何查詢引數，將不會快取資源。 此外，如果您新增隨機數字作為引數的值(例如「398547283745」)，URL就會變成唯一值，而且您會確定AEM系統和熒幕之間的其他快取也無法進行快取。 通常，中間疑犯會是位於Dispatcher、CDN甚至瀏覽器快取前的「清漆」快取。 再次：請勿這麼做。 您確實希望儘可能長時間地快取資源。 快取是您的朋友。 不要殺朋友。

#### 自動失效

>[!WARNING]
>
>這是反模式。 請避免用於數位資產。 嘗試保留Dispatcher的預設設定，>只會讓「.html」檔案自動失效

在短期內，您可以將「.jpg」和「.png」新增到Dispatcher中的自動失效設定。 這表示每當發生失效，所有「.jpg」、「.png」和「.html」都需要重新呈現。
...
Unix時間戳記適合在真實世界中實作。 為了提高可讀性，我們在本教學課程中使用較易閱讀的格式： `2018 31.12 23:59 or fp-2018-31-12-23-59`.

不得將指紋用作查詢引數，因為無法快取具有查詢引數的URL。 您可以使用指紋的選擇器或尾碼。

假設，檔案 `/content/dam/flower.jpg` 具有 `jcr:lastModified` 2018年12月31日日期，23:59。 具有指紋的URL為 `/content/home/jcr:content/par/respi.fp-2018-31-12-23-59.jpg`.

只要參照的資源(`flower.jpg`)檔案未變更。 因此可以無限期地快取它，而且它不是快取殺手。
...
您可能會認為 _應該_...但事實並非如此。由於只變更了影像的二進位檔案，而且未觸及包含頁面，因此不需要重新轉譯HTML標籤。 因此，Dispatcher會提供具有舊指紋的頁面，進而提供舊版本的影像。
...
懶惰的管理員通常會「_將自動失效設為jpgs，並將statfile層級設為0 — 這始終有助於解決各種型別的快取問題_.」 您會在技術論壇中找到該建議，並有助於解決您的失效問題。

到目前為止，我們尚未討論statfile層級。 基本上，自動失效僅適用於相同子樹狀結構中的檔案。 但問題是頁面和資產通常不在相同的子樹狀結構中。 頁面位於以下某個位置 `/content/mysite` 而資產位於下方 `/content/dam`.
...
無論如何 — 我們需要告訴Dispatcher，如果「.jpg」或「.png」變更為允許使用全新URL重新呈現，以使HTML頁面失效。
...
這樣的話，當您的自訂Dispatcher清除代理程式遇到/content/site-a在上變更時，可以輕鬆傳送和失效請求。 `/content/dam/site-a`.

實際上，您告訴Dispatcher讓哪個路徑失效並不重要，只要它是在同一網站、同一「子樹」中。 您甚至不必使用真正的資源路徑。 這也可能是「虛擬」的：
...
2. 監聽器傳送失效請求給Dispatcher。 由於自動失效，我們會在自動失效中傳送哪個路徑並不重要，除非是在網站首頁下 — 或在網站statfile層級中更精確。
...
每個請求都會略過Dispatcher，造成Publish執行個體上載。 更糟的是，在Dispatcher上建立相應的檔案。

因此……您必須檢查影像的jcr：lastModified日期，如果不是預期的日期，則傳回404，而不只是使用指紋做為簡單的快取殺手。 這需要一些時間和CPU週期，在發佈系統上……這是您一開始想要防止的。
...
但在這裡，您可能會面臨另一個警告：具有URL指紋：它會將URL連結至內容。 若不變更URL （亦即更新修改日期）則無法變更內容。 這就是指紋的設計初衷。 但請考量一下，您正在推出新版本，其中包含新的CSS和JS檔案，因此是帶有新指紋的新URL。 您的所有HTML頁面仍會參照舊的指紋URL。 因此，為了讓新版本持續運作，您需要一次讓所有HTML頁面失效，以強制使用新指紋檔案的參照重新呈現。 如果您有多個網站仰賴相同的程式庫，那麼重新呈現可能會相當耗時，而且您無法在此運用 `statfiles`. 因此，請準備好在轉出後檢視發佈系統上的負載尖峰。 您可能會考慮使用具有快取預熱功能的藍綠色部署，或是在您的Dispatcher前面使用TTL型快取……可能性是無限的。

#### 簡短插播

哇 — 需要考慮的細節太多了，對吧？ 而且不易理解、測試及除錯。 此外，還有看似優雅的解決方案。 無可否認，這確實很優雅，但只是從AEM的角度來看。 它與Dispatcher一起變得很麻煩。
...
然後它可能會向Dispatcher發出一些失效請求。 每個包含資產的頁面各一個。
...
如果需要對特定元件執行一些特殊處理，我們會在該路徑上註冊一個專用的servlet，並選擇器來代表元件執行轉換。 我們在這裡以「.respi」作為範例。 選擇器。 建議您持續追蹤全域URL空間中使用的選取器名稱(例如 `/content/dam`)並擁有良好的命名慣例以避免命名衝突。
...
唯一缺少的是品質的值。 元件知道作者輸入的屬性……當標籤轉譯時（例如），可以輕鬆地將它作為查詢引數傳遞給影像轉譯servlet `flower.respi2.jpg?quality=60`：
...
![以選取器傳遞元件屬性](assets/chapter-1/passing-component-properties.png)
...
如果您沒有網頁應用程式防火牆，則必須在Dispatcher或AEM中篩選。 如果您在AEM中執行此動作，請確定
...
這種做法完全沒有幫助。 這些要求實際上是有效的要求。  它們會消耗處理能力，並佔用Dispatcher上快取目錄中的空間。
...
AEM會在此處告知瀏覽器。 「我沒有 `q-41`. 不過您好，您可以問我 `q-40` 「。

這會在交談中新增額外的要求 — 回應回圈，雖然有點額外負荷，但比完全處理要便宜 `q-41`. 而且您可以運用已快取的檔案 `q-40`. 不過您必須瞭解，Dispatcher中未快取302回應，我們討論的邏輯是在AEM中執行。 一遍又一遍。 因此您最好讓電腦更輕薄快速。
...
實際上，每個層級都會有某些安全性。 但請確定您有辦法找出在哪個層級篩選掉請求。 確定您可以直接存取發佈系統、Dispatcher和WAF上的記錄檔，以找出鏈結中的哪個篩選器會封鎖請求。
...
哇……多工緩衝處理器的討論比預期的更長更複雜。 我們缺你個藉口。 但我們覺得必須向您呈現許多方面，包括好方面和壞方面，以便您對於在Dispatcher領域哪些方面有效哪些方面無效形成一些直覺。
...
我們已簡要地提過 _statfile_ 之前。 這與自動失效有關：

Dispatcher檔案系統中所有設定為自動失效的快取檔案，如果其上次修改日期早於 `statfile's` 上次修改日期。

>[!NOTE]
>
>快取檔案是指從使用者端瀏覽器請求該檔案，並最終在檔案系統中建立該檔案的日期，也就是我們所說的上次修改日期。 它不是 `jcr:lastModified` 資源的日期。

statfile的上次修改日期(`.stat`)是Dispatcher收到來自AEM的失效請求的日期。

如果您有多個Dispatcher，這可能會導致奇怪的效果。 您的瀏覽器可以有較新版本的Dispatcher （如果您有一個以上的Dispatcher）。 或者Dispatcher可能會認為其他Dispatcher所發行的瀏覽器版本已過期，且不必要地傳送新復本。 這些效果對效能或功能需求沒有重大影響。 而且當瀏覽器有最新版本時，它們會隨著時間平準。 不過，當您最佳化和偵錯瀏覽器快取行為時，可能會有點令人困惑。 所以請小心。

#### 使用/statfileslevel設定失效網域

當我們推出自動失效功能時，我們說 *全部* 如果有任何變更，且所有檔案都相互依存，則會將檔案視為無效。

這不太準確。 通常，共用相同主導覽根目錄的所有檔案都是相依的。 但一個AEM執行個體可以託管多個網站 —  *獨立* 網站。 不共用共用共用導覽 — 事實上，不共用任何內容。

讓網站B失效不是因為網站A有變更而浪費嗎？ 是的，是的。 而且不一定非要如此。

Dispatcher提供簡單的方法將各個網站分隔開來： `statfiles-level`.
...
`/statfileslevel "0":` 此 `.stat` 檔案會在docroot中建立。 失效網域涵蓋整個安裝，包括所有網站。

無論哪個檔案失效， `.stat` dispatchers docroot最頂端的檔案一律會更新。 所以當您使 `/content/site-b/home`，也是所有檔案 `/content/site-a` 也會失效，因為它們現在已超過 `.stat` 檔案。 當您使無效時，顯然不是您需要的內容 `site-b`.

在此範例中，您寧可設定 `statfileslevel` 至 `1`.

現在，如果您發佈 — 並因此失效 `/content/site-b/home` 或下列任何其他資源 `/content/site-b`，則 `.stat` 檔案建立於 `/content/site-b/`.

以下內容 `/content/site-a/` 則不受影響。 此內容會與 `.stat` 檔案位於 `/content/site-a/`. 我們已建立兩個不同的失效網域。
...
大型安裝的結構通常較為複雜和深入。 常見的方案是依品牌、國家/地區和語言來建構網站。 在這種情況下，您可以將statfileslevel設定得更高。 _1_ 會根據品牌建立失效網域， _2_ 每個國家/地區和 _3_ 依語言區分。
...
設定為 _2_ 這表示在大型網站中，您宣告了 `/canada/en` 和 `/canada/fr` 相依性，但可能不是。 因此，中的每一項失效 `/en` 也會失效 `/fr`. 這會導致快取命中率稍微降低，但還是比提供過時的快取內容更好。
...
現在哪個層級正確？ 這取決於您在網站之間的相依性數量。 您解析來呈現頁面的包含專案會視為「硬相依性」。 我們展示了 _包含專案_ 當我們引入 _Teaser_ 本指南開頭的元件。

_超連結_ 是較柔和的相依關係形式。 您很有可能會在一個網站內建立超連結……而且您的網站之間也不太可能有連結。 簡單的超連結通常不會在網站之間建立相依性。 想像一下您從網站設定為facebook的外部連結……如果在facebook上發生任何變更，您就不需要呈現頁面，反之亦然，對嗎？
...
您可以透過透明化來避免排名下降，您事實上擁有多個具有相同內容的頁面，而且您並未嘗試「博弈」系統(請參閱 [「告訴Google您的頁面當地語系化版本的相關資訊」](https://support.google.com/webmasters/answer/189077?hl=en))，方法是設定 `<link rel="alternate">` 標籤會顯示在每個頁面的標頭區段中，與每個相關頁面相對應：
...
此配置不僅造成許多連結，也造成一些問題。 所需的連結數 _p_ 在 _n_ 語言是 _p x (n<sup>2</sup>-n)_：每個頁面連結至彼此頁面(_n x n_)但不包括本身(_-n_)。 此配置會套用至每個頁面。 如果我們有一個小網站，採用4種語言，每種20頁，則相當於 _240_ 連結。

首先，您不希望編輯器必須手動維護這些連結，因為這些連結必須由系統自動產生。

第二，它們應該是準確的。 每當系統偵測到新的「相對」時，您就會想要將它從其他所有內容相同（但使用不同語言）的頁面連結。

在我們的專案中，新的相對頁面會經常出現。 但是它們並沒有具體化為「替代」連結。 例如，當 `de-de/produkte` 此頁面已發佈在德國網站上，無法立即在其他網站上看到。
...
這應該會導致 `statfiles` 層級為1 — 這會導致過大的失效網域。

重組網站便可修正此問題。 將所有品牌合併到一個共同根下。 但我們當時沒有容量，而那只能提供我們第2級的容量。

我們決定堅持使用第3級，並支付不一定要擁有最新「替代」連結的價格。 為了減輕影響，我們在Dispatcher上執行了一個「死神」cron-job，該作業無論如何都會清除超過1週的檔案。 所以最終所有頁面都會在某個時間點重新呈現。 但這是一個需要分別在每個專案中決定的取捨。

## 結論

我們介紹了Dispatcher一般運作方式的一些基本原則，並舉例說明您可能必須投入更多實施工作才能正確運作以及可能想要進行取捨的地方。

我們未詳述如何在Dispatcher中設定。 我們希望您能先瞭解基本概念和問題，避免過早迷失於主控台。 而且 — 實際的設定工作已完整記錄 — 如果您瞭解基本概念，您應該知道各種交換器的用途。
...
如果立即安裝AEM Author和Publish，拓撲會有點奇怪。 作者會將內容傳送至發佈系統，同時將失效請求傳送至Dispatcher。 由於發佈系統和Dispatcher兩者都是透過佇列與作者脫鉤，計時可能會有點不幸。 在發佈系統上更新內容之前，Dispatcher可以接收來自作者的失效請求。
...
通常，您會透過以下步驟，開始將缺少的標頭重新套用至Apache伺服器中的資源 `mod_headers` 透過使用可由資源路徑與尾碼衍生的資訊。 但這並不總是足夠的。

尤其令人討厭的是，即使使用Dispatcher _未快取_ 瀏覽器的回應來自具有完整標題範圍的發佈系統，而後續回應是由Dispatcher以有限的標題集產生。
...
一般而言，您可能想要快取所有頁面和影像，但在某些情況下則例外。 例如，您要快取PNG影像，但不快取PNG影像以顯示驗證碼（假設每個請求都會變更）。 Dispatcher可能不會將驗證碼辨識為驗證碼……但AEM當然會辨識。 它可以要求Dispatcher不要快取該請求，方法是傳送相應的標頭以及回應：
...
通常，您可以使用Apache的 `mod_expires` 或儲存來自AEM的Cache-Control和Expires Header （如果您需要更多個別的控制項）。

瀏覽器中的快取檔案可以有三個最新層級。

1. _保鮮_  — 瀏覽器可以使用快取的檔案。

2. _可能過時_  — 瀏覽器應該先詢問伺服器快取的檔案是否仍然是最新的，
...
若要判斷瀏覽器快取中的檔案是否仍會與新請求時傳送的檔案相同，瀏覽器可使用 `Last-Modified` 檔案的日期。 瀏覽器會要求伺服器：

&quot;_我有從6月10日起的版本……我需要更新嗎？_「而伺服器可能會回應

&quot;_304 — 您的版本仍是最新版本_」而不重新傳輸資源，或伺服器可透過回應

&quot;_200 — 以下是較新的版本_&quot;，以及HTTP內文中的實際較新內容。

若要讓第二部分正常運作，請務必傳輸 `Last-Modified` 瀏覽器的日期，以便有參考點可要求更新。

我們先前曾說明，當 `Last-Modified` 日期由Dispatcher產生，在不同的請求之間可能有所不同，因為快取檔案（以及日期）是在瀏覽器請求檔案時產生的。 另一種選擇是使用「e-tags」 — 這些數字可識別實際內容（例如，透過產生雜湊代碼），而不是日期。

&quot;[Etag支援](https://adobe-consulting-services.github.io/acs-aem-commons/features/etag/index.html)」來自 _ACS Commons套件_ 會使用此方法。 但這也有代價：由於E-Tag必須以標頭傳送，但雜湊程式碼的計算需要完全讀取回應，因此必須在主要記憶體中完全緩衝回應，才能傳送。 當您的網站很可能有未快取的資源，而且您當然需要留意AEM系統消耗的記憶體時，這可能會對延遲產生負面影響。

如果您使用URL指紋，則可以設定很長的到期日。 您可以在瀏覽器中永遠快取指紋資源。 新版本會以新的URL標示，而舊版本則永遠不需要更新。

我們在推出多工緩衝處理器模式時，使用了URL指紋。 來自的靜態檔案 `/etc/design` (CSS、JS)很少變更，這也使其成為用作指紋的良好候選專案。
...
構成AEM UI的Granite程式庫可以快取相當長的時間。 您也可以在瀏覽器中快取網站靜態檔案（字型、CSS和JavaScript）。 中的偶數影像 `/content/dam` 通常可以快取約15分鐘，因為它們沒有像頁面上的複製文字那樣頻繁地變更。 影像無法在AEM中以互動方式編輯。 它們會先經過編輯和核准，再上傳至AEM。 因此，您可以假設它們不像文字那樣頻繁地變更。
...
但請勿僅依賴AEM。 如果這樣做，您會有類似以下的路徑 `/home.html` 在快取的根目錄中。 這就是Finish或德文或加拿大網站的「首頁」嗎？ 如果有檔案 `/home.html` 在Dispatcher中，Dispatcher如何知道當的失效請求時，這必須失效 `/content/brand/fi/fi/home` 進來。

我們看到一個專案對每個網域都有單獨的docroot。 除錯和維護起來簡直是噩夢 — 實際上，我們從來沒見過它完美無缺地運作。

我們可藉由重新建構快取來解決問題。 所有網域都有單一docroot，當伺服器上的所有檔案都開始使用時，失效請求可以以1:1處理 `/content`.

截斷部分也非常簡單。  AEM由於中的設定而產生了截斷的連結 `/etc/map`.

現在當有請求時 `/home.html` 會點選Dispatcher，首先發生的是套用內部擴展路徑的重寫規則。
...
在檔案系統中，我們現在有了 `/content` — 以路徑為基礎，也可在製作和發佈上找到 — 這有助於進行大量偵錯。 更不用說正確失效 — 這不再是問題。
...
這當然不是客戶看到的情況。 在Dispatcher中，我們註冊了 `ErrorDocuments` 如下所示：
...
現在，AEM系統只需通知Dispatcher發生錯誤，Dispatcher就可以傳送醒目而優美的錯誤檔案版本。

請注意兩件事。

首先， `error-404.html` 永遠是相同的頁面。 因此，沒有類似「您的搜尋」的個別訊息&#x200B;_product_「沒有產生結果」。 我們可以輕鬆地接受這一點。
...
Dispatcher在預設情況下提供資源時不會檢查許可權。 實作方式與以下類似，是有目的的 — 以加速您的公用網站。 如果您想透過登入來保護某些資源，則基本上有三個選項：

1. 在請求點選快取之前Protect資源 — 也就是說，由Dispatcher前面的SSO （單一登入）閘道進行，或當作Apache伺服器中的模組
...
>[!NOTE]
>
>此模式需要 _閘道_ 該 _截獲_ 每個請求並執行實際 _authorization_  — 核准或拒絕對Dispatcher的請求。 如果您的SSO系統是 _驗證者_，只會建立您必須實作選項3之使用者的身分識別。 如果您在SSO系統手冊中閱讀「SAML」或「OAauth」等詞語，這是您必須實施選項3的有力指標。
...
為了緩解這有時稱為「快取失效風暴」的問題，您可以對 `statfile` 解釋。

您可以設定Dispatcher使用 `grace period` 以自動失效。 這會在內部新增一些額外時間至 `statfiles` 修改日期。

假設您的 `statfile` 修改時間為今天12:00，且 `gracePeriod` 設為2分鐘。 然後所有自動失效的檔案在12:01和12:02被視為有效。 它們會在12:02後重新呈現。

參考設定會建議 `gracePeriod` 兩分鐘，理由很充分。 您可能會認為「兩分鐘？ 這幾乎什麼都不是。 我可以輕鬆等待10分鐘讓內容顯示……」  因此，您可能會想要設定更長的時段（假設10分鐘後至少會顯示您的內容）。

>[!WARNING]
>
>這不是方法 `gracePeriod` 正在運作。 寬限期為 _非_ 保證檔案失效，但不會發生失效的時間範圍。 屬於此框架的每個後續失效 _延長_ 時間範圍 — 可以無限長。

讓我們來說明一下 `gracePeriod` 實際上使用的是一個範例：
...
等等……內容永遠不會失效。 每次失效 *範圍* gracePeriod會有效延長寬限時間。 此 `gracePeriod` 專門設計來抵禦失效風暴……但您最終必須外出降雨……因此，請保持 `gracePeriod` 相當短，可避免永遠躲在避難所裡。

#### 確定性寬限期

我們想要介紹另一個您如何經受無效風暴的想法。 這只是個構想。 我們尚未在生產環境中嘗試，但我們發現這個概念相當有趣，足以與您分享這個想法。

此 `gracePeriod` 如果您的規則復寫間隔短於 `gracePeriod`.

替代想法如下：僅在固定時間間隔內失效。 兩者之間的時間長度一律代表提供過時內容。 最終將會發生失效，但一旦「大量」失效，系統會收集到許多失效專案，這樣Dispatcher就可以同時提供一些快取的內容，並讓Publish系統喘口氣。
...
![將失效時間延遲到下一個完整的30秒會增加點選率。](assets/chapter-1/postponing-the-invalidation.png)

*將失效時間延遲到下一個完整的30秒會增加點選率。*
...
任意的失效請求會傳送給Dispatcher，導致所有頁面（包括您最受歡迎一次）失效。

隨後，由於這些頁面非常受歡迎，因此會有來自不同瀏覽器的新傳入請求。 以首頁為例。

由於快取現在無效，所有同時傳入的首頁請求將轉送至產生高負載的發佈系統。

![對空白快取上的相同資源提出平行請求：將請求轉送至發佈](assets/chapter-1/parallel-requests.png)

*對空白快取上的相同資源提出平行請求：將請求轉送至發佈*

使用自動重新擷取，您可在某種程度上減少這種情形。 在自動失效後，大多數失效的頁面仍會實際儲存在Dispatcher上。 它們只是 _已考慮_ 過時。 _自動重新擷取_ 這表示您在起始時仍會持續提供這些過時頁面幾秒鐘 _單一_ 要求發佈系統重新擷取過時內容：
...
若要啟用重新擷取，您必須通知Dispatcher在自動失效後要重新擷取哪些資源。 請記住，您啟動的任何頁面也會自動讓所有其他頁面失效，包括您常用的頁面。
...
當Dispatcher看到這類請求時，就會如常觸發自動失效，並立即將請求排入佇列，以便從發佈系統重新擷取最新內容。

由於我們現在是使用要求內文，因此也需要根據HTTP標準設定content-type和content-length。

Dispatcher還在內部標籤相應的URL，以便知道可以直接傳送這些資源，即使這些資源被自動失效視為無效。
...
Dispatcher會將Publish系統遮蔽在僅用於維護的請求之外，提供一些額外的安全性。 例如，您不想公開 `/crx/de` 或 `/system/console` 公開的URL。
...
如果您這麼做，別忘了永遠將Dispatcher處理常式繫結到Dispatcher的失效URL — 否則，您將無法從AEM傳送失效請求給Dispatcher。
...
幸運的是，在較新版本的Dispatcher中已變更。 現在您也可以使用POSIX規則運算式，而且您可以存取要求的不同部分來定義篩選器。 對於剛開始使用Dispatcher且可能被視為理所當然的人。 但如果您習慣只使用地球，這有點令人吃驚，也很容易被忽視。 除了glob和regexes的語法非常類似。 讓我們比較兩個執行相同動作的版本：
...
_萬用字元模式_，則使用雙引號 `"` 而且您只能使用簡單的預留位置，例如 `*`.

如果您知道這些差異，就認為這無關緊要 — 但若非如此，您就可以輕鬆地混淆引號，花一個晴朗的下午來偵錯您的設定。 現在警告您。

「我認得 `'/url'` 在設定中……但那是什麼 `'/glob'` 在篩選中，您可能會問？

該指示詞代表整個要求字串，包括方法和路徑。 它可以代表

`"GET /content/foo/bar.html HTTP/1.1"`

這是將用來比較您模式的字串。 初學者往往忘記第一部分，亦即 `method` (GET、POST...)。 因此，模式
...
AEM和Dispatcher在多數情況下都是現成可用的功能。 因此，您將找不到有關失效通訊協定的完整Dispatcher通訊協定規格，以在上面建置您自己的應用程式。 此資訊是公開的，但分散於許多資源中。
...
Dispatcher檔案與所有指示說明： [https://helpx.adobe.com/experience-manager/dispatcher/using/dispatcher-configuration.html](https://helpx.adobe.com/experience-manager/dispatcher/using/dispatcher-configuration.html)
...
簡報2018年Potsdam舉行的「未充分認識的內容失效能力」、「adaptTo()」會議 [https://adapt.to/2018/en/schedule/the-underappreciated-power-of-content-invalidation.html](https://adapt.to/2018/en/schedule/the-underappreciated-power-of-content-invalidation.html)
