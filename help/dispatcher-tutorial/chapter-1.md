...
然而，這些資訊的一部分和一部分總是從非常技術的層面開始 — 假設您已經知道要做什麼，因此只提供了關於如何實現您想要什麼的詳細資訊。 我們從未找到任何概念性檔案，說明&#x200B;_的內容，以及為何_，說明您可以和無法對Dispatcher執行哪些操作。
...
您可能聽說過程式設計建議&#x200B;_「首先讓它起作用 — 然後讓它快速起作用」。_. 這並非完全錯誤。 不過，若沒有正確的上下文，則傾向於誤解及未正確套用。
...
Dispatcher模組會運用Apache伺服器托管所提供的功能。 HTML頁面、下載和圖片等資源在Apache檔案系統中儲存為簡單檔案。 就這麼簡單。

檔案名由請求資源的URL衍生。 如果您要求檔案`/foo/bar.html`，則會儲存該檔案，例如在/`var/cache/docroot/foo/bar.html`下。
...
如果URL尾碼為`path/suffix.ext`，則

* `pagename.selectors.html` 建立為資料夾

* `path` 資料夾中的資 `pagename.selectors.html` 料夾

* `suffix.ext` 是資料夾中的檔 `path` 案。注意：如果尾碼沒有副檔名，則不會快取該檔案。
...
1. URL可能會變長。 在本地檔案系統上添加`/docroot`的「path」部分可能很容易超過某些檔案系統的限制。 在Windows上以NTFS執行Dispatcher可能是個難題。 但是，Linux是安全的。

2. URL可包含特殊字元和變母。 這通常對Dispatcher來說不是問題。 但請記住，URL會在您應用程式的許多位置進行解譯。 我們經常看到應用程式的奇怪行為 — 只是為了找出一段很少使用（自訂）的程式碼沒有針對特殊字元進行徹底測試。 如果可以的話，你應該避開它們。 如果你做不到，那就計畫徹底測試。
...
`http://domain.com/home.html` 可快 **取**
...
`http://domain.com/home.html/path/suffix.html` 可快 **取**
...
在AEM中是絕對有效的。 您不會在本機開發電腦上看到任何問題（沒有Dispatcher）。 在UAT或負載測試中，您很可能也不會遇到任何問題。 我們面臨的問題如此微妙，以至於在大多數測試中都漏掉了。  當您處於高峰時段時，它會對您造成嚴重影響，並且您將被限制在解決該問題的時間，可能沒有伺服器訪問權，也沒有修復它的資源。 我們去過……
...
對`home.html/suffix.html`的後續請求返回了有效結果，但由於檔案`home.html` &quot;blocks&quot;檔案系統中的位置，因此無法再次建立`home.html`作為資料夾，因此不快取`home.html/suffix.html`。
...
若您以相反的方式執行，請先請求`home.html/suffix.html`，然後快取`suffix.html`資料夾`/home.html`下。 但是，當您隨後請求`home.html`作為資源時，此資料夾將被檔案`home.html`刪除並替換。
...
因此，快取結果完全是隨機的，且取決於傳入請求的順序。 更棘手的是，您通常有多個調度程式。 而且，效能、快取點擊率和行為可能會因Dispatcher和Dispatcher而有所不同。 如果您想要了解網站為何沒有回應，必須確定您查看的Dispatcher是否有不良的快取順序。 如果您查看的Dispatcher（幸運的是）請求模式較有利，您將會在嘗試尋找問題時迷失方向。
...
最後一章列出許多例外，當Dispatcher無法快取請求時。 但還有更多事情要考慮：僅僅因為Dispatcher _可以_&#x200B;快取請求，並不一定表示它&#x200B;_應該_。

重點是：快取通常很容易。 Dispatcher只需儲存回應的結果，並在下次傳入非常相同的請求時傳回。 右? 錯了！

難點在於快取的&#x200B;_無效_&#x200B;或&#x200B;_沖洗_。 當資源已變更時，Dispatcher需要找出，且需要重新呈現。
...
現在，使用者上傳並啟用該檔案的新版本。 最終，無效請求會從AEM傳送至Dispatcher,
...
移除`image.png`和`image.thumb.png`，以及符合該模式的所有其他轉譯。
...
與上傳至AEM的影像或其他二進位檔案不同，HTML頁面不是單獨的動物。 它們生活在亂堆中，它們通過超連結和引用彼此高度互連。 簡單的連結無害，但當我們討論內容參考時，就會變得棘手。 頁面上無處不在的頂端導覽或前導是內容參考。
...
** 匯入首頁。首頁由Dispatcher儲存為靜態.html檔案，包括預告，且是檔案中的標題。
...
「冬季特殊」頁面尚未轉譯，因此Dispatcher上沒有靜態版本，因此會與新預告一起顯示，因為新預告會應要求以新鮮呈現。

您可能會認為，當資源變更時，Dispatcher會在演算及排清已使用此資源的所有頁面時，持續追蹤其所接觸的每個資源。 但Dispatcher不會轉譯頁面。 呈現由發佈系統執行。 Dispatcher不知道哪些資源會放入轉譯的.html檔案中。

還沒說服？ 您可能認為&#x200B;*&quot;必須有實作某種相依性追蹤&quot;*&#x200B;的方法。 有，或者更準確地說，*是*。 公報3 AEM的曾曾曾祖父在&#x200B;_session_&#x200B;中實施了一個依賴追蹤器，用於呈現頁面。
...
Dispatcher中所有建立日期早於statfile的檔案，都已在上次啟動（和失效）之前呈現，因此被視為「無效」。 檔案系統中仍有這些檔案，但Dispatcher會忽略這些檔案。 它們是「過時的」。 每當對過時資源提出請求時，Dispatcher都會要求AEM系統重新呈現頁面。 然後，新呈現的頁面將儲存在檔案系統中 — 現在有新的建立日期，並且它再次刷新。
...
您可以問它為什麼叫「.stat」？ 也許不是&quot;無效&quot; 可以想像，在您的檔案系統中包含該檔案有助於Dispatcher確定可以靜態&#x200B;**&#x200B;提供哪些資源 — 就像從靜態Web伺服器提供資源一樣。 這些檔案不再需要以動態方式呈現。

然而，這個名字的真正本質並不是暗喻。 它衍生自Unix系統調用`stat()`，該調用返回檔案的修改時間（除其他屬性外）。
...
而HTML頁面則具有高度的相依性。 因此，您會對這些套用自動失效。 這是Dispatcher中的預設設定。 屬於無效資源的所有檔案都會實際刪除。 此外，結尾為「.html」的檔案會自動失效。

Dispatcher會決定檔案副檔名，決定是否套用自動失效配置。

可設定自動失效的檔案結尾。 理論上，您可以包含自動失效的所有擴充功能。 但請記住，這個代價非常高。 您不會無意中傳送過時資源，但傳送效能會因過度失效而大幅降低。

假設您實作的配置是動態轉譯PNG和JPG，並依賴其他資源執行。 您可能希望將高解析度影像重新縮放為與Web相容的較小解析度。 雖然你也會改變壓縮率。 此示例中的解析度和壓縮率不是固定常數，而是使用影像的元件中的可配置參數。 現在，如果此參數已變更，您必須使影像無效。
...
2. 如果您的代理傳送HTTP標題`CQ-Action-Scope: ResourceOnly`，表示此單一無效請求不會觸發自動失效。 這([https://github.com/cqsupport/webinar-dispatchercache/tree/master/src/refetching-flush-agent/refetch-bundle](https://github.com/cqsupport/webinar-dispatchercache/tree/master/src/refetching-flush-agent/refetch-bundle))一段代碼可能是您自己的複製代理的良好起點。

3. `ResourceOnly`，僅防止自動失效。若要實際執行必要的相依性解決和無效判定作業，您必須自行觸發無效判定請求。 您可以檢查套件Dispatcher排清規則([https://adobe-consulting-services.github.io/acs-aem-commons/features/dispatcher-flush-rules/index.html](https://adobe-consulting-services.github.io/acs-aem-commons/features/dispatcher-flush-rules/index.html))，以了解如何實際發生這一情況的靈感。
...
資產會上傳至AEM的DAM區域，且只會在回應式影像元件中參考&#x200B;__。
...
該元件分為兩部分；第一部分呈現影像的HTML標籤，第二部分「假設」參照影像的二進位資料。 由於這是具有回應式設計的現代網站，我們並未轉譯簡單的`<img src"…">`標籤，而是在`<picture/>`標籤中轉譯一組影像。 我們會針對每部裝置將兩個不同的影像上傳至DAM，並從影像元件參照這些影像。
...
現在由你決定這是一種模式還是一種反模式。 也許你已經有了一些好的想法，來緩解上述問題？ 很好。 那麼，你將迫切希望看到其他項目如何解決這些問題。
...
您是否曾看過`?ck=398547283745`等查詢參數？ 它們稱為快取殺手(「ck」)。 其思想是，如果新增任何查詢參數，則不會快取資源。 此外，如果您新增隨機數字作為參數的值(例如&quot;398547283745&quot;),URL就會變成唯一，您確定AEM系統與您的畫面之間沒有其他快取可快取。 通常中間的疑似情況是Dispatcher、CDN或甚至瀏覽器快取前的「清漆」快取。 再次：別這樣。 您確實希望盡可能多地快取您的資源。 快取是你的朋友。 別殺朋友。
...
如果企業擁有者抱怨其變更無法以足夠快的速度在即時網站上實現，則這種模式非常容易實施。 但這只能給你一些時間，來想出更複雜的解決方案。
...
指紋不得作為查詢參數，作為具有查詢參數的URL   無法快取。 您可以使用選取器或尾碼作為指紋。

假設檔案`/content/dam/flower.jpg`的日期為2018年12月31日23:59。 `jcr:lastModified`指紋的URL為`/content/home/jcr:content/par/respi.fp-2018-31-12-23-59.jpg`。

只要引用的資源(`flower.jpg`)檔案未更改，此URL就會保持穩定。 因此，它可以快取無限時間，而且不是快取殺手。

注意，此URL必須由回應式影像元件建立並提供。 這不是現成可用的AEM功能。

這是基本概念。 不過，有些細節很容易被忽視。

在我們的範例中，元件於23:59轉譯並快取。 現在影像已經變更，假設00:00。  元件&#x200B;_會_&#x200B;在其標籤中生成新的指紋URL。

您可能認為它&#x200B;_應該_...但它不是。由於只變更了影像的二進位檔，而且未接觸包含頁面，因此不需要重新轉譯HTML標籤。 因此，Dispatcher會以舊指紋提供頁面，進而提供舊版影像。
...
懶惰管理員通常會&quot;_將自動失效設為jpgs，並將statfile級別設為零 — 這一定有助於解決各種_&#x200B;的快取問題。&quot; 您會在技術論壇中找到該建議，這有助於解決失效問題。

到目前為止，我們還沒有討論statfile級別。 基本上，自動失效僅適用於相同子樹狀結構中的檔案。 但問題是頁面和資產通常不會存在於相同的子樹狀結構中。 頁面位於`/content/mysite`下方，而資產位於`/content/dam`下方。

「statfile level」定義子樹的根節點在什麼深度。 在上方的範例中，層級會是&quot;2&quot;(1=/content, 2=/mysite,dam)

將statfile層級「降低」為0的想法基本上是將整個/content樹狀結構定義為一個且唯一的子樹狀結構，以讓頁面和資產即時位於相同的自動失效網域中。 因此，我們只能在大樹上一級（在docroot &quot;/&quot;處）。 但是，只要某些內容發佈，伺服器上的所有網站就會自動失效，甚至是完全不相關的網站。 相信我們：從長遠來看，這是個壞主意，因為整體快取點擊率會嚴重降低。 您只能希望AEM伺服器擁有足夠的火力，不需要快取即可執行。
...
這樣，當您的自訂Dispatcher排清代理遇到`/content/dam/site-a`上的變更時，可以輕鬆傳送和失效請求至/content/site-a。
...
與任何已失效的資源無關。 記憶? 我們只使「虛擬」資源失效，並依賴自動失效來將「首頁」視為無效。 影像本身可能永遠不會刪除&#x200B;_phyallic_。 因此，快取會增長，並且會增長。 當影像變更並啟動時，會在Dispatcher的檔案系統中取得新的檔案名稱。
...
[版本控](https://adobe-consulting-services.github.io/acs-aem-commons/features/versioned-clientlibs/index.html) 制Clientlib是使用此方法的模組。

但在這裡，您可能會面臨另一個警告：它會將URL系結至內容。 若不變更URL（亦即更新修改日期），則無法變更內容。 這就是指紋最初的設計。 但請考慮，您正在推出新版本，其中包含新的CSS和JS檔案，因此包含新指紋的新URL。 所有HTML頁面仍會參照舊的指紋URL。 因此，若要讓新版本持續運作，您必須一次使所有HTML頁面無效，以強制重新轉譯並參照新指紋的檔案。 如果您有多個網站依賴相同的資料庫，這可能需要大量重新轉譯 — 而此處您無法運用`statfiles`。 因此，請準備好在轉出後看到發佈系統上的負載峰值。 您可以考慮將快取加熱為藍綠色的部署，或是Dispatcher前面的TTL型快取……有無限的可能性。

#### 短暫的休息

這是很多細節要考慮，對吧？ 而且它拒絕被輕易理解、測試和調試。 而且，這都是為了一個看似優雅的解決方案。 誠然，它很優雅 — 但只從AEM的視角看。 和Dispatcher一起，這會變得令人討厭。

但是，這仍無法解決一個基本警告，如果影像在不同頁面上多次使用，系統會在這些頁面下快取這些影像。 快取的協同作用不大。
...
接著，它可能會向Dispatcher發出許多無效請求。 每個包含資產的頁面各一個。
...
如果需要對特定元件執行一些特殊處理，我們會在該路徑上註冊一個專用的servlet，並選擇器代表元件進行轉換。 我們在這裡以「回應」為榜樣。 選取器。 最好追蹤全域URL空間（例如`/content/dam`）上使用的選取器名稱，並有良好的命名慣例，以避免命名衝突。
...
那麼，我們可以解決避免內部內容引用的問題，因為外部元件在內部資源的渲染中添加的值或資訊很少，所以可以很容易地在一組靜態選擇器中對它進行編碼，這些靜態選擇器控制單個資源的表示。
...
所缺少的只是品質的值。 元件知道作者輸入的屬性……它可以在呈現標注時輕鬆地傳遞給影像呈現servlet作為查詢參數，如`flower.respi2.jpg?quality=60`:
...
這又會略過快取，並在發佈系統上建立負載。 所以，這可能是個壞主意。 您只能篩選一小部分參數，即可緩解此問題。 僅允許`q-20, q-40, q-60, q-80, q-100`。

#### 使用選取器時篩選無效請求

減少選取器數量是個好開始。 根據經驗，您應始終將有效參數的數量限制為絕對最小值。 如果您這樣做，您甚至可以巧妙地利用AEM外的Web應用程式防火牆，使用一組靜態的篩選器，而不需要深入了解底層AEM系統，以保護您的系統：
...
這裡AEM告訴瀏覽器。 「我沒有`q-41`。 但是，你可以問我`q-40` &quot;。

這會在對話中增加額外的請求 — 回應回圈，這雖然有點開銷，但比在`q-41`上完全處理要便宜。 您也可以利用`q-40`下已快取的檔案。 不過，您必須了解，Dispatcher中不會快取302個回應，我們討論的是AEM中執行的邏輯。 一次又一次。 所以你最好把它變得苗條快。
...
如果您有專為Web安全而設計的Web應用程式防火牆設備或「WAF」，則您絕對應該利用這些功能。 但您可能會發現，WAF是由對內容應用程式知之甚少的人員操作，他們會篩選有效請求或允許傳遞太多有害請求。 也許您會發現，WAF的運營人員被分配到具有不同班次和發佈時間安排的不同部門，溝通可能不像直接隊友那樣緊密，而且您並不總是能及時得到變化，這意味著最終您的開發和內容速度會受到影響。
...
1. 當元件在主要請求循環內被請求時，客戶端將呈現HTML標籤
...
我們將此模式稱為&#x200B;_反轉後台程式_。 反轉後台程式必須以影像資源為基礎，才能有所有好的快取失效屬性。
...
對後衛的討論比預期的更長，也更複雜。 我們欠你個藉口。 但我們覺得有必要向大家介紹很多方面 — 好的和壞的方面 — 這樣你就能對Dispatcher領域的哪些方面有效、哪些方面不有效產生一些直覺。
...
我們之前曾簡短提及過&#x200B;_statfile_。 它與自動失效有關：

如果Dispatcher檔案系統中設定為自動失效的所有快取檔案的上次修改日期早於`statfile's`上次修改日期，則這些檔案會被視為無效。

>[!NOTE]
>
>我們討論的上次修改日期是快取檔案，即從客戶端的瀏覽器中請求檔案並最終在檔案系統中建立的日期。 不是資源的`jcr:lastModified`日期。
...
當我們引入自動失效和statfile時，我們說，當有任何更改且所有檔案都相互依存時，*所有*&#x200B;檔案都被視為無效。

那不太準確。 通常，共用共同主導覽根的所有檔案都相互依存。 但一個AEM例項可以托管多個網站 — *獨立的*&#x200B;網站。 不共用共用導覽 — 事實上，不共用任何內容。

使站點B無效不是浪費，因為站點A有更改嗎？ 是的。 這不一定非要這樣。

Dispatcher提供將網站彼此分隔的簡單方法：`statfiles-level`。

它是一個數字，它定義了檔案系統中的哪個級別，兩個子樹被視為「獨立的」。

讓我們看看statfilelevel為0的預設案例。

![/statfilelevel &quot;0&quot;:_  _.stat_ _是在資料夾中建立。無效域涵蓋整個安裝，包括所有站點](assets/chapter-1/statfile-level-0.png)

`/statfileslevel "0":` 檔 `.stat` 案會在資料夾中建立。無效網域涵蓋整個安裝，包括所有網站。

無論檔案失效，Dispatcher最上方的`.stat`檔案一律會更新。 因此，當使`/content/site-b/home`無效時，`/content/site-a`中的所有檔案也會無效，因為它們現在比docroot中的`.stat`檔案舊。 當您使`site-b`無效時，顯然不需要什麼。

在此範例中，您寧可將`statfileslevel`設為`1`。

現在，如果您發佈 — 並因此使`/content/site-b/home`或`/content/site-b`下的任何其他資源無效，則會在`/content/site-b/`建立`.stat`檔案。

`/content/site-a/`以下的內容不受影響。 此內容將與`/content/site-a/`處的`.stat`檔案進行比較。 我們已建立兩個不同的無效網域。
...
大型安裝的結構通常更複雜、更深。 一個常見的方案是按品牌、國家和語言來構建網站。 在這種情況下，您可以將statfilelevel設定得更高。 _1_ 會根據每個品牌、每個國 _家/地區_ 和每種語言 _建立3_ 個失效網域。
...
前者需要&#x200B;_2_&#x200B;的`statfileslevel`，而後者需要&#x200B;_3_。

不是理想情況。 如果將其設為&#x200B;_3_，則子分支`/home`、`/products`和`/about`之間的較小網站內將無法自動失效。

將其設為&#x200B;_2_&#x200B;表示，在較大的網站中，您會聲明`/canada/en`和`/canada/fr`相依，但這可能不相依。 因此，`/en`中的每個失效也將使`/fr`失效。 這會導致快取點擊率稍微降低，但仍優於傳送過時快取內容。
...
哪個是正確的級別？ 這取決於您在網站之間的相依性數量。 您為轉譯頁面而解析的包含項目視為「硬相依性」。 在本指南的開頭引入&#x200B;_Teaser_&#x200B;元件時，我們演示了這樣的&#x200B;_includion_。

__ 超連結是相依性較軟的形式。很可能，您會在一個網站內建立超連結……而且您的網站之間不太可能有連結。 簡單超連結通常不會在網站之間建立相依性。 想想您從網站設定到facebook的外部連結……如果facebook上有任何變更，您就不需要轉譯頁面，反之亦然？
...
透明化、您實際上有多個頁面具有相同內容，以及您並未嘗試透過在每個頁面的標題區段中為每個相關頁面設定`<link rel="alternate">`標籤，來「遊戲」系統（請參閱[&quot;告訴Google您頁面的當地語系化版本&quot;](https://support.google.com/webmasters/answer/189077?hl=en)），借此防止排名下滑：
...
此方案不僅產生了許多連結，也產生了一些問題。 _n_&#x200B;語言中&#x200B;_p_&#x200B;所需的連結數為&#x200B;_p x(n<sup>2</sup>-n)_:每個頁面彼此連結(_n x n_)，但本身(_-n_)除外。 此配置會套用至每個頁面。 如果我們有4種語言的小型網站，共有20頁，每個頁面都等於&#x200B;_240_&#x200B;連結。

首先，您不希望編輯器必須手動維護這些連結 — 這些連結必須由系統自動產生。

其次，它們應該是準確的。 每當系統偵測到新的「相對」時，您就會想從所有其他具有相同內容（但使用不同語言）的頁面將其連結。

在我們的專案中，新的相對頁面經常出現。 但它們並沒有成為「替代」的連結。 例如，當`de-de/produkte`頁面在德國網站上發佈時，它不會立即顯示在其他網站上。
...
`shiny-brand` 在法 `marque-brillant` 國和德 `blitzmarke` 國被叫：
...
這原本會將`statfiles`層級設為1 — 這會導致失效網域過大。
...
我們說明了Dispatcher一般運作方式的一些基本原則，並舉出一些範例，說明您可能需要付出更多實作努力才能正確運作，以及您可能想要取捨之處。
...
更有責任的設定是在&#x200B;_收到內容後，從Publish系統_&#x200B;傳送無效請求。 文章「[從發佈例項使Dispatcher快取失效](https://helpx.adobe.com/experience-manager/dispatcher/using/page-invalidate.html#InvalidatingDispatcherCachefromaPublishingInstance)」說明了詳細資訊。
...
通常，您會開始使用`mod_headers`可由資源路徑和尾碼衍生的資訊，將遺失的標題重新套用至Apache伺服器中的資源。 但這並不總是足夠。

尤其令人討厭的是，即使是Dispatcher第一次&#x200B;_uncached_&#x200B;回應來自具有完整標題的Publish系統，而後續回應則是由Dispatcher產生，且標題集有限。
...
快取控制和編譯指示是官方的HTTP標題，會傳播至上層快取層（例如CDN）並由其解譯。 `Dispatcher`標題只是Dispatcher不要快取的提示。 它可用來指示Dispatcher不要快取，同時仍允許上層快取層快取。 實際上，很難找到這樣的案例。 但我們肯定有，某處。
...
通常，您會使用Apache的`mod_expires`，或儲存來自AEM的Cache-Control和Expires Header（如果您需要更個別的控制項），以靜態方式執行此操作。
...
要確定瀏覽器快取中的文檔是否與新請求時傳送的文檔相同，瀏覽器可以使用文檔的`Last-Modified`日期。 瀏覽器會詢問伺服器：

&quot;_我有6月10日的版本……是否需要更新？_&quot;伺服器可以以

&quot;_304 — 您的版本仍為最新版本_&quot;，無需重新傳輸資源，或伺服器可回應

&quot;_200 - HTTP標題中有較新的版本_&quot;，而HTTP內文中有實際較新的內容。

若要讓第二個部分正常運作，請務必將`Last-Modified`日期傳送至瀏覽器，使其具有要求更新的參考點。

我們先前說明，當`Last-Modified`日期由Dispatcher產生時，它會因不同請求而異，因為快取檔案（及其日期）是在瀏覽器要求檔案時產生。 另一種方法是使用「e-tags」，即這些數字可識別實際內容（例如透過產生雜湊代碼），而非日期。

_ACS Commons Package_&#x200B;中的&quot;[Etag Support](https://adobe-consulting-services.github.io/acs-aem-commons/features/etag/index.html)&quot;使用此方法。 然而，這也帶來了一個價格：由於E-Tag必須以標頭形式發送，但計算哈希代碼需要完全讀取響應，因此響應必須在主記憶體中完全緩衝，才能傳送。 當您的網站很可能沒有資源時，這可能會對延遲造成負面影響，當然您必須留意AEM系統使用的記憶體。

如果您使用URL指紋，則可以設定很長的到期日。 您可以在瀏覽器中永久快取指紋資源。 新版本會標示新URL，而舊版本永遠不需要更新。

引入假離線模式時，我們使用了URL指紋。 來自`/etc/design`(CSS、JS)的靜態檔案很少變更，也是用作指紋的理想候選檔案。

對於一般檔案，我們通常會設定固定配置，例如每30分鐘重新檢查HTML、每4小時重新檢查影像等。

瀏覽器快取對製作系統非常有用。 您想要盡可能在瀏覽器中快取，以增強編輯體驗。 很可惜，最昂貴的資產，即無法快取html頁面……這些頁面應該會在作者上經常變更。

組成AEM UI的Granite程式庫可快取相當長的時間。 您也可以在瀏覽器中快取網站靜態檔案（字型、CSS和JavaScript）。 即使是`/content/dam`中的影像，通常也可快取約15分鐘，因為它們不會像頁面上的複製文字一樣經常變更。 在AEM中不會以互動方式編輯影像。 先完成編輯並核准，再上傳至AEM。 因此，您可以假設它們的變更頻率不如文字。
...
但不要只依賴AEM。 若是如此，快取的根目錄中會有`/home.html`之類的路徑。 現在，這是「終點」的「家」，還是德語，還是加拿大的網站？ 如果Dispatcher中有檔案`/home.html`,Dispatcher如何知道當`/content/brand/fi/fi/home`的無效請求傳入時，該檔案必須失效。

我們已看到一個專案，每個網域各有不同的配對。 調試和維護是個噩夢 — 實際上，我們從未見過它完美運行。

我們可以重新構建快取，以解決問題。 所有網域都有單一Docroot，且無效請求可以處理1:1，因為伺服器上的所有檔案都是以`/content`開頭的。

截斷部分也很容易。  AEM因`/etc/map`中的設定而產生截斷連結。

現在，當請求`/home.html`點擊Dispatcher時，首先發生的情況是套用在內部展開路徑的重寫規則。
...
在檔案系統中，我們現在有基於`/content`的純路徑，這些路徑也可在「作者」和「發佈」中找到，這有助於調試很多。 更別提正確的失效 — 這已不是問題。
...
這當然不是客戶看到的。 在Dispatcher中，我們已註冊`ErrorDocuments`，如下所示：
...
首先，`error-404.html`一律為相同頁面。 因此，沒有「您對「_producten_」的搜尋未產生結果」之類的個別訊息。 我們很容易就能忍受。

其次……如果我們發現內部伺服器錯誤，或更嚴重的是，我們遇到AEM系統中斷，就無法要求AEM轉譯錯誤頁面，對吧？ `ErrorDocument`指令中定義的必要後續請求也會失敗。 我們解決此問題的方法是運行cron-job，該cron-job會通過`wget`定期從其定義的位置提取錯誤頁，並將錯誤頁儲存到`ErrorDocuments`指令中定義的靜態檔案位置中。
...
>[!NOTE]
>
>此模式需要&#x200B;_網關_,_截取每個請求並執行實際的_&#x200B;授權&#x200B;_— 授予或拒絕Dispatcher請求。_&#x200B;如果您的SSO系統是&#x200B;_authenticator_，則僅建立必須實施選項3的用戶的身份。 如果您在SSO系統手冊中閱讀了「SAML」或「OAauth」等詞語，這是您必須實作選項3的強項指標。


**選項2**。通常，「不快取」是個壞主意。 如果您這麼做，請確定流量和排除的敏感資源數量很少。 或者，請確定Publish系統中已安裝一些記憶體內快取，讓Publish系統可以處理產生的負載，這在本系列第III部分有更多說明。

**選項3**。「權限敏感快取」是一種有趣的方法。 Dispatcher正在快取資源，但在傳送資源之前，會詢問AEM系統是否可能快取。 這會從Dispatcher建立額外的請求給Publish，但如果頁面已快取，通常會讓Publish系統無法重新轉譯頁面。 不過，此方法需要一些自訂實作。 若需詳細資訊，請參閱文章[敏感權限快取](https://helpx.adobe.com/experience-manager/dispatcher/using/permissions-cache.html)。
...
若要緩解此「快取失效風暴」（有時稱為）的問題，您對`statfile`解譯的嚴格程度可能會較低。

您可以將Dispatcher設為使用`grace period`來自動失效。 這會在內部為`statfiles`修改日期增加一些額外時間。

假設您的`statfile`的修改時間為今天12:00，而您的`gracePeriod`設為2分鐘。 之後，所有自動失效的檔案都會在12:01和12:02被視為有效。 12時02分以後再轉譯。

參考配置建議2分鐘的`gracePeriod`，理由充分。 你可能會想&quot;兩分鐘？ 那幾乎沒什麼。 我可以輕鬆等待10分鐘，讓內容顯示……」  因此，您可能會想要設定更長的時段 — 假設10分鐘，假設您的內容至少在這10分鐘之後顯示。

>[!WARNING]
>
>這不是`gracePeriod`的運作方式。 寬限期為&#x200B;_not_&#x200B;保證檔案失效後的時間，但沒有發生失效的時間範圍。 落在此幀&#x200B;_elanded_&#x200B;時間幀內的每個後續失效 — 這可能是無限長的。

讓我們說明`gracePeriod`如何實際運作範例：
...
等等，內容永遠不會失效。 在&#x200B;*內的每個無效*&#x200B;有效地延長了寬限期。 `gracePeriod`設計用於抵御失效風暴……但您最終必須出雨……因此，請保持`gracePeriod`相當短，以防止永遠躲在掩體中。

#### 確定性寬限期

我們想介紹另一個概念，如何抵御無效風暴。 這只是個主意。 我們在生產中沒有嘗試過，但我們發現這個概念足夠有趣，可以和大家分享。

如果您的常規複製間隔比`gracePeriod`短，`gracePeriod`可能會變成無法預測的長。

另一種想法如下：僅在固定時間間隔內失效。 中間的時間一律代表提供過時內容。 失效最終會發生，但會收集許多無效判定作業，使其變成一個「大量」失效，讓Dispatcher有機會同時提供某些快取內容，並為Publish系統提供喘息的空間。

實作如下所示：

您使用會在失效發生後執行的「自訂失效指令碼」（請參閱參考）。 此指令碼將讀取`statfile's`上次修改日期，並將其捨入到下一個間隔停止。 Unix shell命令`touch --time` ，讓您指定時間。
...
透過自動重新擷取，您可能會在一定程度上減少影響。 自動失效後，大部分已失效的頁面仍會實際儲存在Dispatcher中。 它們只&#x200B;_被視為_&#x200B;過時。 _自_ 動重新擷取：表示您在向發佈系統起始單一 _請求_ 以重新擷取過時內容時，仍會提供這些過時頁面數秒：
...
Dispatcher可讓Publish系統免受僅用於維護目的之請求的影響，提供額外的安全性。 例如，您不想將`/crx/de`或`/system/console` URL公開給公用者。
...
將Dispatcher作為篩選器的另一個替代方法是在`dispatcher.any`中設定篩選指令
...
B版使用單引號`'`來標示&#x200B;_規則運算式模式_。 &quot;Any character&quot;是使用`.*`表示的。

_全域模式_，相反地使用雙引號， `"` 而您只能使用類似的簡單預留位 `*`置。

如果您知道這種差異，那就無關緊要了 — 但如果不知道，您可以輕鬆混搭引號，然後在陽光明媚的下午對您的配置進行調試。 現在你被警告了。

&quot;我可識別配置中的`'/url'`..但您可能會問的篩選器中是什麼？`'/glob'`
...
>[!WARNING]
>
>更精密的設定會將篩選規則分割為多個檔案，這些檔案由主`dispatcher.any`組態檔包含。 但新檔案不會引入新的命名空間。 因此，如果一個檔案中有「001」規則，另一個檔案中有「001」規則，您會收到錯誤訊息。 更有理由提出語義強的名字。
...
`CQ-Handle: <path-pattern>`  — 要失效的內容資源路徑。注意，`<path-pattern>`實際上是「path」，而非「pattern」。

`CQ-Action-Scope: ResourceOnly`  — 可選：如果設定了此標題，則不 `.stat` 會接觸檔案。
...
更多最佳化提示與秘訣：[https://helpx.adobe.com/experience-manager/kb/optimizing-the-dispatcher-cache.html#use-ttls](https://helpx.adobe.com/experience-manager/kb/optimizing-the-dispatcher-cache.html#use-ttls)
...
演示2018年在波茨坦召開的「內容失效的未充分認識的力量」、「adapTo()」會議[https://adapt.to/2018/en/schedule/the-underappreciated-power-of-content-invalidation.html](https://adapt.to/2018/en/schedule/the-underappreciated-power-of-content-invalidation.html)
