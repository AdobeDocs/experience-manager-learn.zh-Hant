...
這在里絕對有效AEM。 您不會在本地開發電腦上看到任何問題（沒有Dispatcher）。 在UAT或負載測試中，您很可能也不會遇到任何問題。 我們面臨的問題非常微妙，以至於它在大多數test中溜走了。  在您處於高峰時段時，它會給您帶來沈重的打擊，而且您的解決時間有限，可能沒有伺服器訪問權限，也沒有修復它的資源。 我們去過那裡……

所以……什麼問題？

`home.html` 檔案系統中可以是檔案或資料夾。 不是同時發AEM生。

如果您請求 `home.html` 首先，將其建立為檔案。
...
「冬季特殊」頁尚未呈現，因此Dispatcher上沒有靜態版本，因此在請求時新呈現時，會隨新預告一起顯示。
...
現在，這是一種模式還是一種反模式，由你決定。 也許你已經有了一些好的想法，想如何減輕上面解釋的問題？ 很好。 那麼，你應該急著看看其他項目是如何解決這些問題的。
...
如果使用URL指紋，則可以設定很長的過期日期。 您可以在瀏覽器中永久快取指紋資源。 新版本會用新URL標籤，而舊版本則不必更新。
...
比如說， `statfile` 修改時間為今天12:00，您 `gracePeriod` 設定為2分鐘。 然後，所有自動失效的檔案在12:01和12:02被視為有效。 12點02分後重新渲染。
...
[helpx.adobe.com - Dispatcher配置](https://helpx.adobe.com/experience-manager/dispatcher/using/dispatcher-configuration.html)
