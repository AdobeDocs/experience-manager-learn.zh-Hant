...
使用&#x200B;**擴充點將** Widget **新增到內容片段編輯器中的** RTE`rte`。 使用`rte`擴充點的`getWidgets()`方法新增一或多個Widget。 會透過按下`{`特殊鍵以開啟內容功能表選項來觸發，然後選取所需的Widget以載入自訂對話方塊UI。
...
### 在`discountCodes`中新增`App.js`路由{#add-widgets-route}
...
+ 使用`addDiscountCode`物件的`guestConnection`函式提供要執行的RTE指令。 在此案例中，會在RTE中插入實際折扣代碼的`insertContent`指示和HTML程式碼片段。
