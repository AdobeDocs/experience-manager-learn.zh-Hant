...
RTE徽章最常見的使用案例是搭配使用 [RTE Widget](https://developer.adobe.com/uix/docs/services/aem-cf-editor/api/rte-widgets/). 這可讓RTE Widget插入到RTE中的內容不可編輯。

通常，與Widget相關聯的徽章會用於新增具有外部系統相依性，但 _內容作者無法修改_ 插入的動態內容以保持完整性。 它們只能當作整個專案移除。

此 **徽章** 新增至 **RTE** 在內容片段編輯器中使用 `rte` 延伸點。 使用 `rte` 擴充點的 `getBadges()` 方法新增一或多個徽章。

此範例說明如何新增名為的Widget _Large Group Bookings客戶服務_ 尋找、選擇並新增WKND探險特有客戶服務詳細資訊，例如 **代表姓名** 和 **電話號碼** 在RTE內容中。 使用徽章功能 **電話號碼** 已建立 **不可編輯** 但WKND內容作者可以編輯代表名稱。

此外， **電話號碼** 樣式不同（藍色），這是badges功能的額外使用案例。

為了簡單起見，此範例使用 [AdobeReact光譜](https://react-spectrum.adobe.com/react-spectrum/index.html) 框架來開發Widget或對話方塊UI和硬式編碼WKND客戶服務電話號碼。 若要控制內容的非編輯及不同樣式方面，請 `#` 字元用於 `prefix` 和 `suffix` 徽章定義的屬性。

## 延伸點

此範例延伸至擴充點 `rte` 在內容片段編輯器中，將徽章新增至RTE。
...
下列範例會建立 _Large Group Bookings客戶服務_ Widget. 按下 `{` 鍵，RTE Widget內容功能表隨即開啟。 藉由選取 _Large Group Bookings客戶服務_ 自訂強制回應視窗會從快顯選單中開啟。

從強制回應視窗新增所需的客戶服務編號後，徽章會將 _不可編輯的電話號碼_ 並將其樣式設定為藍色。
...
+ 在此範例中， `#` 字元可用來定義此徽章的邊界，意即RTE中由所包圍的任何字串 `#` 會視為此徽章的例項。

另請參閱RTE Widget的重要詳細資訊：
...
Widget或對話方塊UI是使用 [AdobeReact光譜](https://react-spectrum.adobe.com/react-spectrum/index.html) 框架。

新增客戶服務詳細資料時，React元件程式碼會以括住電話號碼變數 `#` 註冊徽章字元，可將其轉換為徽章，例如 `#${phoneNumber}#`，因此使其不可編輯。

以下為的關鍵重點 `LargeBookingsCustomerService` 程式碼：

+ UI會使用React Spectrum元件轉譯，例如 [下拉式方塊](https://react-spectrum.adobe.com/react-spectrum/ComboBox.html)， [ButtonGroup](https://react-spectrum.adobe.com/react-spectrum/ButtonGroup.html)， [按鈕](https://react-spectrum.adobe.com/react-spectrum/Button.html)
+ 此 `largeGroupCustomerServiceList` 陣列具有代表名稱與電話號碼的硬式編碼對應。 在實際案例中，此資料可從AdobeAppBuilder動作或外部系統，或是自行開發或雲端提供者型API閘道擷取。
+ 此 `guestConnection` 已使用進行初始化 `useEffect` [React鉤點](https://react.dev/reference/react/useEffect) 和managed as component state。 它可用來與AEM主機通訊。
...
+ 若要建立 **不可編輯的電話號碼** 使用徽章， `#` 特殊字元會新增在之前和之後 `phoneNumber` 變數，例如 `...<div><p>Phone Number: #${phoneNumber}#</strong></p></div>`.
