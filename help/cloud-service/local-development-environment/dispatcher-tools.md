...
可選擇使用`httpd -t`指令碼驗證Dispatcher和Apache Web伺服器設定（透過`validate`） （請勿與`validator`可執行檔混淆）。 `validate`指令碼提供執行[的](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/implementing/content-delivery/validation-debug.html?lang=zh-Hant)三個階段`validator`的便利方式。
