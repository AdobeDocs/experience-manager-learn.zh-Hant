...
透過將AEM體驗片段匯出至Adobe Target做為HTML選件的功能，您可以將AEM的易用性和威力，結合Target中強大的自動化智慧(AI)和機器學習(ML)功能，以大規模測試並個人化體驗。
...
WKND網站正計畫宣佈 **冰鞋節挑戰賽** 在美國各地，他們希望網站用戶註冊參加每個州的試鏡。 身為行銷人員，您已獲指派在WKND網站首頁上執行促銷活動的工作，其中包含與使用者位置相關的橫幅訊息以及事件詳細資料頁面的連結。 讓我們探索WKND網站首頁，了解如何根據使用者目前的位置，為使用者建立並提供個人化體驗。
...
1. 行銷人員與AEM內容編輯器起始WKND SkateFest行銷活動討論，並詳細說明需求。
   * ***需求***:在WKND網站首頁上提升WKND SkateFest促銷活動，為來自美國每個州的訪客提供個人化內容。 在首頁輪播下方新增內容區塊，其中包含背景影像、文字及按鈕。
...
1. 從AEM將體驗片段匯出為Adobe Target作為選件之後，行銷人員可以使用這些選件在Target中建立活動。 根據WKND網站SkateFest行銷活動，行銷人員需要建立並提供個人化體驗給來自每個州的WKND網站訪客。 若要建立體驗鎖定目標活動，行銷人員需要識別對象。 為了進行WKND SkateFest活動，我們需要根據他們造訪WKND網站的所在位置，建立50個不同的受眾。
   * [對象](https://experienceleague.adobe.com/docs/target/using/introduction/target-key-concepts.html#section_3F32DA46BDF947878DD79DBB97040D01) 定義活動的目標，並用於可使用目標的任何位置。 Target對象是一組已定義的訪客條件。 選件可以鎖定在特定對象（或區段）。 只有屬於該對象的訪客才會看見鎖定他們的體驗。  例如，您可以將優惠方案傳送給由使用特定瀏覽器或來自特定地理位置的訪客所組成的對象。
   * 安 [選件](https://experienceleague.adobe.com/docs/target/using/introduction/target-key-concepts.html#section_973D4CC4CEB44711BBB9A21BF74B89E9) 是在促銷活動或活動期間，在您的網頁上顯示的內容。 當您測試網頁時，會測量每個體驗在您位置中使用不同選件的成功程度。 選件可以包含不同類型的內容，包括：
      * 影像
      * 文字
      * **HTML**
         * *HTML選件用於此情境的活動*
...
1. 導覽至 **選件** 標籤並搜尋「WKND」選件。 您應該可以看到從AEM匯出為HTML選件的體驗片段變體清單。 每個選件都對應至一個狀態。 例如， *加州WKND冰鞋節* 是提供給來自加州的WKND網站訪客的選件。
...
1. 按一下 **新增規則>地理**.
1. 按一下 **選擇**，然後選取下列其中一個選項：
   * 國家/地區
   * **狀態** *（為WKND Site SkateFest促銷活動選擇狀態）*
   * 城市
   * 郵遞區號
   * 緯度
   * 經度
   ...
   **地理**  — 根據使用者的地理位置，包括其國家/地區、州/省、城市、郵遞區號、DMA或行動電信業者，使用受眾來鎖定使用者。 地理位置參數可讓您根據訪客的地理位置來定位活動和體驗。 此資料會根據訪客的IP位址，與每個Target請求一併傳送。 選取這些參數就像任何定位值一樣。

   >[!NOTE]
   >訪客的IP位址會隨mbox請求而傳遞，每次造訪（工作階段）傳遞一次，以解析該訪客的地理定位參數。

1. 將運算子選為 **matches**，請提供適當的值(例如：加州)和 **儲存** 您的變更。 在本例中，請提供州名。
...
1. 按一下 **建立活動** ，然後選取 **體驗鎖定** 活動類型。
1. 選取 **Web** 通道，並選擇 **可視化體驗撰寫器**.
1. 輸入 **活動URL** 按一下 **下一個** 以開啟可視化體驗撰寫器。

   WKND網站首頁發佈URL:http://localhost:4503/content/wknd/en.html

   ![體驗鎖定目標活動](assets/personalization-use-case-1/target-activity.png)

1. 針對 **可視化體驗撰寫器** 要載入，請啟用 **允許載入不安全指令碼** 並重新載入頁面。
...
1. 若要將對象新增至VEC，請按一下 **新增體驗鎖定目標** 在「對象」下，選取WKND-California對象，然後按一下 **下一個**.

   ![VEC](assets/personalization-use-case-1/vec-select-audience.png)

1. 按一下VEC內的WKND網站頁面，選取要新增WKND-California對象選件的HTML元素，然後選擇 **替換為** 選項，然後選取 **HTML選件**.

   ![體驗鎖定目標活動](assets/personalization-use-case-1/vec-selecting-div.png)

1. 選取 **加州WKND冰鞋節** HTML選件 **加利福尼亞WKND** 選件中的對象，請選取UI並按一下 **完成**.
1. 您現在應該能夠看到 **加州WKND冰鞋節** HTML選件已新增至WKND-California對象的WKND網站頁面。
1. 重複步驟7-10，將體驗鎖定目標新增至其他狀態，然後選擇對應的HTML選件。
1. 按一下 **下一個** 若要繼續，您可以看到對象與體驗的對應。
1. 按一下 **下一個** 移至「目標與設定」。
1. 選擇您的報表來源並識別活動的主要目標。 對於我們的案例，我們選取「報表來源」作為 **Adobe Target**，將活動測量為 **轉換**、「已檢視頁面的動作」，以及指向「WKND SkateFest詳細資料」頁面的URL。
...
1. 暫留在目前的活動名稱上，您可將其重新命名為 **WKND冰鞋節 — 美國**，然後 **儲存並關閉** 您的變更。
1. 在「活動詳細資訊」畫面中，請務必 **啟動** 您的活動。

   ![啟動活動](assets/personalization-use-case-1/activate-activity.png)

1. 您的WKND SkateFest促銷活動現已對所有WKND網站訪客啟用。
1. 導覽至 [WKND站點首頁](http://localhost:4503/content/wknd/en.html)，而您應該能夠根據您的地理位置查看WKND SkateFest優惠方案(*狀態：加州*)。

   ![活動QA](assets/personalization-use-case-1/wknd-california.png)

### Target活動QA

1. 在 **活動詳細資料>概觀** ，按一下 **活動QA** 按鈕，您就可以取得所有體驗的直接QA連結。

   ![活動QA](assets/personalization-use-case-1/activity-qa.png)

1. 導覽至 [WKND站點首頁](http://localhost:4503/content/wknd/en.html)，您應該就能根據您的地理位置（狀態）查看WKND SkateFest優惠方案。
...
在本章中，內容編輯器能建立所有內容以支援Adobe Experience Manager中的WKND SkateFest促銷活動，並將其匯出為「HTML選件」，以便根據使用者的地理位置建立體驗鎖定目標。
