...
# 動態媒體經典主工作流程與預覽資產{#main-workflow}
...
>[!TIP]
>
>不要手動添加資料夾。 請改為從FTP執行上傳，並使用&#x200B;**Include Subfolders**&#x200B;選項，在Dynamic Media Classic中重新建立檔案夾結構。
...
在第一個示例中，到映像&quot;Backd_A&quot;的虛擬路徑是`http://sample.scene7.com/is/image/s7train/BackpackA`。 「影像伺服器」修飾元會將影像調整為250像素的寬度（從wid=250），並使用Lanczos內插演算法重新調整影像，它會隨著調整大小（從resMode=sharp2）而銳利化。
...
請記住，Dynamic Media Classic工作流程的最終產品是指向資產的URL。 URL可能指向個別影像、影像集、回轉集或其他影像集系列或視訊。 您需要擷取該URL並加以處理，例如編輯HTML，讓`<IMG>`標籤指向Dynamic Media Classic影像，而不是指向來自您目前網站的影像。
